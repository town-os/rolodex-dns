...
- **NXDOMAIN stopped meaning "I have no record of that type".** The authoritative path's only lookup was keyed by *(name, type)*, so a typed miss and an absent name were indistinguishable and every miss left as NXDOMAIN. A host with an A record and no AAAA therefore answered NOERROR and NXDOMAIN for the same name in one lookup pair — `host dns.home` prints that as two NXDOMAIN lines under one successful address — and the zone apex was worse, serving its own SOA while denying that its own name existed. RFC 1035 §3.1 and RFC 2308 §2 make these different assertions: NXDOMAIN says the *name* does not exist, and it is correct only when nothing lives at the name or beneath it. Everything else is NODATA, NOERROR with an empty answer section, and that now includes empty non-terminals — a name holding no records of its own but having descendants. The consequence was never confined to the queried name: under RFC 8020 a resolver caching NXDOMAIN may synthesize it for every name *beneath*, so one AAAA query for a v4-only host could erase its whole subtree from that resolver's view.

- **Negative answers carry the zone's SOA.** Both rcodes now put it in the authority section (RFC 2308 §3), so the MINIMUM field — capped by the record's own TTL — tells a resolver how long the absence may be cached instead of leaving every one of them to pick its own default. The SOA is found by walking up from the queried name rather than read at the zone name that made the server authoritative, because those are routinely different: a managed zone is derived from the last two labels of a stored FQDN, so a record at `dns.home.` registers `dns.home.` as a zone in its own right, matched ahead of the `home.` that actually holds the SOA. Reading the apex from that hint finds nothing, which would have stripped the negative TTL from the commonest shape there is — a host one label under its zone apex.

- **A query type with no `RecordKind` no longer denies the name.** `map_query_type_to_kind` returns `None` for CAA, NAPTR, ANY and anything else outside the stored set, and the whole local lookup was gated on it — so those queries skipped the database entirely and returned NXDOMAIN for a name never looked up. They now take the same existence test as any other type, and they follow a **CNAME** at that name, which the same gate had been skipping: RFC 1034 §3.6.2 makes an alias the answer to every type asked at its name except CNAME itself, so the old behaviour told an ACME client that an aliased name published no CAA policy rather than sending it to the target that holds one.

- **The LAN fallback consults both namespaces before denying a name.** A trusted local client resolving a scope-owned TLD sees the union of the global table and the owning scope, and reaches the fallback only after a *type-keyed* global miss — so a dual-homed name with a global A and no AAAA missed globally, missed in the scope, and was called nonexistent while its A answered from the global table one query later.

- **A name a wildcard matches is no longer denied.** RFC 4592 §2.2.1 has the wildcard *establish* the name, so a query for a type it does not carry is NODATA — the old NXDOMAIN contradicted the positive answer the same wildcard gave one query earlier. It matters more under DNSSEC, where that NXDOMAIN is not merely wrong but unprovable: the proof needs an NSEC covering `*.<parent>`, and no such record can exist while the wildcard does, so the response would go out one record short and be rejected. The NODATA is proved with the wildcard's own NSEC, since the queried name is not literally in the chain. A wildcard still matches only one label, so a name two levels below it is established by nothing and stays NXDOMAIN.

- **A TLD owned by a different network scope is still denied unconditionally**, and deliberately so. It is the one place the server returns NXDOMAIN for a name it knows about: NODATA would assert the name is real and only the type missing, which is exactly what would let one scope enumerate a sibling network's names by watching which came back NODATA instead of NXDOMAIN.

### Features

- **DNSSEC is served, not merely produced.** `SignZone` stored RRSIG rows that no query path ever fetched, so a signed zone answered every positive query bare and every negative one with a naked rcode. A validating resolver reads a missing signature in a zone that publishes DNSKEYs as *stripped* rather than as unsigned, so such a zone was bogus on every answer while every signing test stayed green. Positive answers now carry the RRSIGs covering the RRset (RFC 4035 §3.1.1), gated on the client's DO bit and on the zone actually holding keys — a signature nobody asked for roughly triples an A answer, and a large answer to a small question is the amplification shape the recursion CIDRs exist to close.

- **Authenticated denial of existence.** Signing now builds and signs the zone's NSEC chain in the same pass, and negatives carry the proof: NODATA the queried name's own NSEC, whose type bitmap lists what is present; NXDOMAIN the link covering the name *and* the link covering `*.<closest encloser>`, without which RFC 4035 §5.4 has a validator reject the denial because a wildcard might have synthesized an answer. The SOA is signed alongside, since a mixed authority section fails as a whole. This required an NSEC stored format, an RFC 4034 §4.1.2 type-bitmap encoder, and canonical §6.1 name ordering — none of which existed; the signing loop's existing sort is byte-lexicographic and would have built a chain whose ranges do not tile the zone, which fails silently as "correct denial, called bogus".

- **A signed zone stays signed.** Signing is a snapshot, and the first `AddRecord` after it invalidated two things: the new RRset had no signature, and the NSEC chain still described the zone as it was — so the zone served a *signed proof that its own new record did not exist*, invisibly to whoever added it. Record mutations now mark their zone, a re-sign loop drains the marks every 30 seconds (off the mutation path, because signing is O(zone) and records arrive in bursts at boot), and until it does the stale proof is **withheld** rather than served: that leaves a validator at *insecure*, where a stale chain leaves it at *bogus* and takes the zone down. The mark is dropped at the *start* of a pass rather than the end, so a mutation landing while signing is in flight re-marks the zone instead of having its mark wiped by a pass that never saw it — that record would otherwise sit outside the chain the pass had just published, denied by it. A failed pass re-marks. The operation moved to `src/zone_signer.rs` so the loop and the RPC share one implementation.

- **Forwarders name their transport, and plaintext DNS is one of them.** `forwarders:` and `SetForwarders` entries parse into one `Forwarder` (`src/forwarder.rs`) that carries its transport: a bare `ip:port` is plaintext UDP exactly as before, and a scheme selects `tcp://`, `tls://` (DoT), `https://` (DoH) or `quic://` (DoQ). The `name@ip` authority carries both halves an encrypted upstream needs — the address to dial and the name to validate its certificate against — so nothing has to resolve a hostname before it can reach a resolver. Which tier of `auto` an entry lands in is now derived from the forwarder rather than from the config key it was written under, so a DoH upstream typed into `forwarders:` is tried first because it is encrypted. That is also what makes encrypted upstreams configurable at runtime at all: `resolution.secure_upstreams` is read once at startup and has no gRPC setter, so on a network filtering outbound `:53` the one tier that worked was also the one tier nothing could reconfigure without restarting the box's only resolver.

- **DNS-over-QUIC upstreams (RFC 9250).** A DoQ client on quinn, alongside the existing DoH and DoT ones. Two details are load-bearing rather than cosmetic: the DNS message ID is zeroed on the wire as §4.2.1 requires — QUIC streams already correlate the response, and a conforming server may reject a non-zero ID — and the caller's ID is restored before the answer is returned, because everything above matches answers to queries by it. The stream FIN is sent after the query for the same reason: a server is entitled to wait for it before answering.

- **A forwarder that keeps failing stops being tried.** Each carries a circuit breaker: three consecutive *transport* failures open it for 30s, then one query is let through to see whether it recovered, and a success closes it outright. Health is recorded from the transport rather than the rcode, because a SERVFAIL that arrived is a working resolver answering one question badly and evicting it over a single broken zone would be worse than no breaker at all. This is what makes a dead tier cheap: `auto` walks the whole chain before it can answer SERVFAIL, so on a network that black-holes a transport every query used to pay the full per-forwarder timeout for every address in two tiers that were never going to answer — six seconds of a seven-and-a-half-second failure, spent re-establishing what the previous three queries had already proved.

- **`TOWNOS_CONTRACT.md` (new)** records everything crossing the boundary with Town OS, the install image and ttyforce, in both directions — the gRPC surface, the forwarder grammar, the addresses each repository writes independently, and what rolodex requires back. `make check-townos-sync` verifies it against whatever checkouts are on the machine, with nothing pinned to a revision, and runs as part of `make lint`.

### Metrics

- `rolodex_dns_answers_total{source}` gains **`authoritative_nodata`**, counted apart from `authoritative_nxdomain` because the two make opposite assertions about the name and only one lets a resolver conclude anything about the names beneath it. Counting them together is how the original bug stayed invisible — the rate of authoritative negatives never changed, only their meaning.
- `rolodex_dns_authoritative_negative_total{reason}` (new) says what the lookup *found*: `name_absent`, `type_absent`, `unsupported_type`, `scope_hidden`. The answer-source label says which stage produced a negative but not what it saw, and those are different operational questions — `unsupported_type` climbing means a record type the server ought to support.
- `rolodex_dns_denial_proofs_withheld_total` (new) counts negatives served without their NSEC proof because the zone's signatures were stale: the signal that a signed zone is currently answering unproven.

- `rolodex_dns_upstream_skipped_total{server}` (new) counts the upstream exchanges *not* attempted because a forwarder's circuit breaker was open. It is the tier-walk cost that is no longer being paid: a forwarder on a network that black-holes its transport appears here instead of in `upstream_queries_total`, once per query it would otherwise have stalled.

### Testing

- **`tests/nodata_test.rs`** (new) drives every authoritative-negative path over a real UDP and TCP socket after mutating through the gRPC control plane, with each case paired to its control — a name with nothing at or beneath it, asked in the same zone by the same path. Without that pair, a server answering NODATA for everything would satisfy the positive half while claiming every name in the zone exists.
- **`tests/dnssec_serving_test.rs`** (new) checks that a response is verifiable *by its recipient*, re-deriving verification from the published DNSKEY and re-deriving NSEC ranges against the name asked about. Its three control pairs are DO against non-DO, signed zone against unsigned, and a proof served against one deliberately withheld.
- Unit tests pin the NSEC primitives longhand against the RFC rather than against the encoder that produced them — bit order, the §4.3 worked example, window trimming, canonical ordering *with its control that plain string order disagrees*, and that the generated chain covers every absent name exactly once and every present name not at all.

- **`tests/forwarder_transport_test.rs`** (new) pins the two properties unification is for, each with its control: that an encrypted upstream typed into the plaintext list reaches the encrypted tier (which the old shape could not express at all), and that a black-holed forwarder stops being *dialled* — asserted as a connection count against a loopback socket that never replies, since a test that it eventually SERVFAILs passes with or without the breaker.

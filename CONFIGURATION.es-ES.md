...
**DNSBL bloquea por nombre**, y se comprueba antes de cualquier resolución externa. Viene desactivada por defecto con la lista de proveedores vacía, así que no se consulta nada y ningún nombre llega a manos del operador de una lista de bloqueo hasta que añades proveedores.
...
**Se puede nombrar un certificado que aún no se ha emitido.** Apuntar `cert_path`/`key_path` a un fichero que no existe solo es un fallo duro cuando `auto_self_signed` está desactivado. Con él activado, la escucha arranca con material generado y el sondeo de arriba adopta el par real en cuanto aterriza. Eso es lo que permite escribir estas rutas antes de que haya corrido lo que emite el certificado — el caso corriente en una máquina cuya CA se crea después de que arranque el resolutor, donde la alternativa es reiniciar el único resolutor de la máquina una vez que el fichero existe.

**DoT, DoH y DoQ se reconfiguran en tiempo de ejecución**, mediante `SetDotConfig` / `SetDohConfig` / `SetDoqConfig`. Las direcciones de enlace, las rutas del certificado y la lista de SAN pueden cambiarse en un servidor en marcha, y `Get*Config` informa de lo que está realmente enlazado. El YAML de abajo es la configuración de arranque; no es la única vía de entrada, y deja de ser la autoridad en cuanto el servidor está en pie.

**HTTP/3 es un segundo escucha, y viene apagado.** `doh.enable_h3` abre uno en la dirección y el puerto de DoH sobre UDP, compartiendo el certificado del escucha TCP. El puerto es el mismo porque ahí es donde lo sitúan los dos mecanismos de descubrimiento: una cabecera `Alt-Svc` en cada respuesta DoH, para clientes ya conectados, y `alpn=h2,h3` en la designación DDR, para clientes que no se han conectado nunca. Un bind QUIC que falla hace fallar el transporte entero en vez de dejar h2 solo: un escucha que prometía HTTP/3 y servía h2 es un fallo que ningún cliente puede ver.

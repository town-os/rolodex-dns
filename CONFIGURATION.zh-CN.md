...
`secure_upstreams` 只在启动时读取一次。要在一台正在跑的机器上更换加密层
——在这种网络上，那正是你最不敢重启的那台机器——请改用带 scheme 的转发器
把同一批上游推上去：
...
它们会落在安全层，是因为它们是加密的，而不是因为它们从哪个键送进来。
DoQ（`quic://…:853`）在这里同样可用。
...
```yaml
forwarders:                       # forward 模式下唯一的上游；在 auto 模式中，
  - "8.8.8.8:53"                  # 每一条会落在其传输方式所决定的层级
  - "8.8.4.4:53"
  # - "tcp://8.8.8.8:53"                                  # 明文 TCP
  # - "tls://cloudflare-dns.com@1.1.1.1:853"              # DoT   -> 安全层
  # - "https://cloudflare-dns.com@1.1.1.1/dns-query"      # DoH   -> 安全层
  # - "quic://dns.adguard.com@94.140.14.14:853"           # DoQ   -> 安全层
...
**转发器带着自己的传输方式，层级也就跟着它走。** 裸写的
`ip:port` 一如既往就是明文 UDP，所以既有的配置文件照旧可以解析。
加上 scheme 就会选到别的传输方式，而 `name@ip` 这个 authority
同时给出要拨号的地址与要用来验证证书的名称——因此加密上游
依然不需要任何 DNS 就能启动。层级归属因此是由这个转发器**是什么**
推导出来的，而不是由它被写在哪个配置键底下：写在 `forwarders:`
里的加密条目会在安全层被尝试，而指向局域网的 `public_fallback`
条目会被当成本地转发器。完整的文法见
[转发器规格](README.zh-CN.md#转发器规格)。

这对下面那个部署形状最重要：`secure_upstreams` 没有设置方法，而且
只在启动时读取一次，所以在此之前，在过滤掉 `:53` 的网络上唯一能用的
那一层，同时也是不重启就什么都改不了的那一层。用 `tls://`、`https://`
或 `quic://` 规格调用 `set-forwarders`，就能在线上换掉它。

**一再失败的转发器会停止被尝试。** 连续三次*传输层*失败会为那个
转发器打开一个 30 秒的断路器，接着放一个查询过去看看它恢复了没有，
成功就把它关上。已经回到手上的 SERVFAIL 不算——那是一台正常运作的
解析器把某一个问题答坏了。`forward` 模式从不跳过，因为在那里转发器
就是全部的上游。断路器替你省下来的交换次数，看
`rolodex_dns_upstream_skipped_total{server}`。
...
签名时也会建立并签署该区域的 **NSEC 链**，因此这个区域不只证明什么存在，也证明什么不存在。签名是被取代，而不是累积。RSA（算法 8）在生成密钥时就会被拒绝——`ring` 无法生成 RSA 密钥——而 NSEC3 只会被验证，永远不会被生成。

**你不需要在每次变更记录后都重跑 `sign-zone`。** 一次变动会给它的区域做上标记，而重新签名循环每 30 秒清掉这些标记。在那之前，该区域的否定答案会**不带**证明送出，而不是带着过期的证明：不带证明只让验证器停在 insecure，带着早于这次变动的链则让它判定 bogus，整个区域就此下线。`rolodex_dns_denial_proofs_withheld_total` 就是这段空窗的指标。手动重跑 `sign-zone` 会立刻强制跑一轮，那也是换密钥之后你会想做的事。
...
**HTTP/3 是第二个监听器，而且默认关闭。** `doh.enable_h3` 会在 DoH 的地址与端口上以 UDP 开启一个，并共用 TCP 监听器的证书。端口相同，是因为两种发现机制都说它在那里：对已经连接的客户端，是每个 DoH 响应上的 `Alt-Svc` 标头；对完全还没连接过的客户端，则是 DDR 指定记录中的 `alpn=h2,h3`。失败的 QUIC 绑定会让整个传输失败，而不是单独留下 h2——一个承诺了 HTTP/3 却提供 h2 的监听器，是没有任何客户端看得见的失败。
...
| 已签名区域的否定答案没有带 NSEC 记录，而 `rolodex_dns_denial_proofs_withheld_total` 正在往上爬 | 有记录被新增或删除，该区域正在等待重新签名循环（最多 30 秒）。送出过期的链只会更糟——那会否定掉刚刚才加进去的那条记录。若始终不消退，就是签名失败了；日志会说原因。`sign-zone` 可以强制跑一轮 |
| 某个转发器完全不再被查询，而 `rolodex_dns_upstream_skipped_total` 正在替它计数 | 它的断路器打开了——连续三次传输层失败，之后每次重新开启 30 秒，直到有一个探测查询成功为止。那是传输在失败，不是答案不好：rcode 永远不会打开断路器 |
| 加密转发器出现证书错误 | 证书是对着 `name@ip` 里的那个 authority 验证的；省略 `name@` 则改为对该地址自己的 IP SAN 验证。`tls://1.1.1.1:853` 与 `tls://cloudflare-dns.com@1.1.1.1:853` 是两种不同的检查 |
| 有 A 记录的名称对 AAAA 回答 NXDOMAIN | 已修正——那现在是 NODATA（NOERROR，答案区段为空）。若你仍看得到，表示那个名称并不在本服务器具权威的区域底下，NXDOMAIN 是上游来的 |

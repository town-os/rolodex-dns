...
`secure_upstreams` 只在啟動時讀取一次。要在一台正在跑的機器上更換加密層
——在這種網路上，那正是你最不敢重啟的那台機器——請改用帶 scheme 的轉發器
把同一批上游推上去：
...
它們會落在安全層，是因為它們是加密的，而不是因為它們從哪個鍵送進來。
DoQ（`quic://…:853`）在這裡同樣可用。
...
forwarders:                       # forward 模式下唯一的上游；在 auto 模式中，
  - "8.8.8.8:53"                  # 每一筆會落在其傳輸方式所決定的層級
  - "8.8.4.4:53"
  # - "tcp://8.8.8.8:53"                                  # 純文字 TCP
  # - "tls://cloudflare-dns.com@1.1.1.1:853"              # DoT   -> 安全層
  # - "https://cloudflare-dns.com@1.1.1.1/dns-query"      # DoH   -> 安全層
  # - "quic://dns.adguard.com@94.140.14.14:853"           # DoQ   -> 安全層
...
**轉發器帶著自己的傳輸方式，層級也就跟著它走。** 裸寫的
`ip:port` 一如既往就是純文字 UDP，所以既有的設定檔照舊可以解析。
加上 scheme 就會選到別的傳輸方式，而 `name@ip` 這個 authority
同時給出要撥接的位址與要用來驗證憑證的名稱——因此加密上游
依然不需要任何 DNS 就能啟動。層級歸屬因此是由這個轉發器**是什麼**
推導出來的，而不是由它被寫在哪個設定鍵底下：寫在 `forwarders:`
裡的加密項目會在安全層被嘗試，而指向區域網路的 `public_fallback`
項目會被當成本地轉發器。完整的文法見
[轉發器規格](README.zh-TW.md#轉送器規格)。

這對下面那個部署形狀最重要：`secure_upstreams` 沒有設定方法，而且
只在啟動時讀取一次，所以在此之前，在過濾掉 `:53` 的網路上唯一能用的
那一層，同時也是不重啟就什麼都改不了的那一層。用 `tls://`、`https://`
或 `quic://` 規格呼叫 `set-forwarders`，就能在線上換掉它。

**一再失敗的轉發器會停止被嘗試。** 連續三次*傳輸層*失敗會為那個
轉發器打開一個 30 秒的斷路器，接著放一個查詢過去看看它恢復了沒有，
成功就把它關上。已經回到手上的 SERVFAIL 不算——那是一台正常運作的
解析器把某一個問題答壞了。`forward` 模式從不跳過，因為在那裡轉發器
就是全部的上游。斷路器替你省下來的交換次數，看
...
簽章時也會建立並簽署該區域的 **NSEC 鏈**，因此這個區域不只證明什麼存在，也證明什麼不存在。簽章是被取代，而不是累積。RSA（演算法 8）在產生金鑰時就會被拒絕——`ring` 無法產生 RSA 金鑰——而 NSEC3 只會被驗證，永遠不會被產生。

**你不需要在每次變更記錄後都重跑 `sign-zone`。** 一次變動會替它的區域做上記號，而重新簽署迴圈每 30 秒清掉這些記號。在那之前，該區域的否定答案會**不帶**證明送出，而不是帶著過期的證明：不帶證明只讓驗證器停在 insecure，帶著早於這次變動的鏈則讓它判定 bogus，整個區域就此下線。`rolodex_dns_denial_proofs_withheld_total` 就是這段空窗的指標。手動重跑 `sign-zone` 會立刻強制跑一輪，那也是換金鑰之後你會想做的事。
...
| 已簽章區域的否定答案沒有帶 NSEC 記錄，而 `rolodex_dns_denial_proofs_withheld_total` 正在往上爬 | 有記錄被新增或刪除，該區域正在等待重新簽署迴圈（最多 30 秒）。送出過期的鏈只會更糟——那會否定掉剛剛才加進去的那筆記錄。若始終不消退，就是簽署失敗了；日誌會說原因。`sign-zone` 可以強制跑一輪 |
| 某個轉發器完全不再被查詢，而 `rolodex_dns_upstream_skipped_total` 正在替它計數 | 它的斷路器打開了——連續三次傳輸層失敗，之後每次重新開啟 30 秒，直到有一個探測查詢成功為止。那是傳輸在失敗，不是答案不好：rcode 永遠不會打開斷路器 |
| 加密轉發器出現憑證錯誤 | 憑證是對著 `name@ip` 裡的那個 authority 驗證的；省略 `name@` 則改為對該位址自己的 IP SAN 驗證。`tls://1.1.1.1:853` 與 `tls://cloudflare-dns.com@1.1.1.1:853` 是兩種不同的檢查 |
| 有 A 記錄的名稱對 AAAA 回答 NXDOMAIN | 已修正——那現在是 NODATA（NOERROR，答案區段為空）。若你仍看得到，表示那個名稱並不在本伺服器具權威的區域底下，NXDOMAIN 是上游來的 |

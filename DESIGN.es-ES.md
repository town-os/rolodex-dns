...
Los pasos 5, 6 y sus equivalentes con ámbito terminan todos en una **negativa autoritativa**, y cuál de ellas es la decide el *nombre*, nunca el tipo por el que se preguntó:

- **NXDOMAIN** afirma que el nombre consultado no existe. Solo se devuelve cuando no hay nada en el nombre ni por debajo de él (`Database::name_occupied`, o `scoped_name_occupied` dentro de un ámbito; la reserva LAN del paso 5 exige que ambos estén vacíos, porque un cliente local de confianza resuelve la unión de la tabla global y el ámbito propietario, y solo llega hasta ahí tras un fallo global *con clave de tipo*).
- **NODATA** —NOERROR con la sección de respuesta vacía— afirma que el nombre existe y que el tipo consultado no. Se devuelve para cualquier otro fallo autoritativo: un nombre que guarda registros de otros tipos, y un no-terminal vacío que no guarda ninguno propio pero tiene descendientes.

Ambas llevan el SOA del ápice de la zona envolvente en la **sección de autoridad** (RFC 2308 §3), hallado subiendo desde el nombre consultado (`Database::closest_soa`) en vez de leerse en el nombre de zona que hizo autoritativo a este servidor. No son el mismo nombre: una zona gestionada se deriva de las dos últimas etiquetas de un FQDN almacenado, así que un registro en `dns.home.` inscribe `dns.home.` como zona por derecho propio — emparejada antes que el `home.` que de verdad guarda el SOA — y un ápice leído en esa pista no encuentra nada, despojando del TTL negativo a la forma más común que existe: un host una etiqueta por debajo del ápice de su zona. El ascenso se salta la reserva por comodín, de modo que un comodín no pueda aportar un SOA que no sea el de la propia zona. Su MINIMUM, acotado por el TTL del registro, es como un resolutor aprende cuánto tiempo puede cachear la ausencia; una zona sin SOA en el ápice responde igualmente, y la negativa simplemente no lleva ninguno. Ambas se marcan además como autoritativas — el bit AA es lo que le dice a un resolutor que la negativa merece creerse en vez de merecer preguntársela a otro.

**Una zona firmada demuestra sus negativas.** En una zona con claves, un cliente que haya puesto el bit DO recibe además los registros NSEC que niegan el nombre y los RRSIG que van sobre ellos, junto al SOA y su propia firma — véase [Servicio de DNSSEC](#servicio-de-dnssec) para saber cómo se elige la prueba y cuándo se retiene. Una zona sin claves, o un cliente que no lo pidió, se lleva el SOA y nada más; no hay con qué demostrar nada ni nadie que lo quiera.

Esta distinción no es cosmética, y colapsarla fue un defecto real: como la única búsqueda de la ruta autoritativa está indexada por *(nombre, tipo)*, un fallo de tipo era indistinguible de un nombre ausente y todo fallo se quedaba en NXDOMAIN. Un equipo con un registro A y sin AAAA respondía por tanto NOERROR y NXDOMAIN para el mismo nombre en un mismo par de búsquedas — una autocontradicción, y una cuyo radio de daño se extiende más allá del nombre consultado: bajo el RFC 8020, un resolutor que cachea NXDOMAIN para un nombre puede sintetizar NXDOMAIN para **todos los nombres por debajo de él**, así que una sola consulta AAAA de un equipo solo-v4 puede borrar su subárbol entero de la vista de ese resolutor. El ápice de zona era el caso más afilado: respondía NOERROR por su propio SOA y NXDOMAIN por su propio A.

Dos excepciones deliberadas:

- **Un tipo de consulta que no se corresponde con ninguna clase de registro almacenable** (`map_query_type_to_kind` devuelve `None` — CAA, NAPTR, ANY y cualquier otra cosa fuera de `RecordKind`) se saltaba antes la base de datos por completo, negando un nombre que nunca se llegó a buscar. Ahora pasa la misma prueba de existencia que cualquier otro tipo: NODATA cuando el nombre existe, NXDOMAIN cuando no. También sigue un **CNAME** que haya en ese nombre, algo que la compuerta se saltaba antes junto con todo lo demás — el RFC 1034 §3.6.2 hace de un alias la respuesta a todo tipo que se pregunte en su nombre salvo el propio CNAME, así que el comportamiento antiguo le decía a un cliente ACME que un nombre con alias no publicaba ninguna política CAA en vez de mandarlo al destino que sí tiene una. La negativa, cuando la hay, se cuenta bajo su propia razón `unsupported_type`, porque es el único caso en el que la respuesta es una propiedad de la pregunta y no de la zona.
- **Un nombre con el que casa un comodín existe.** El RFC 4592 §2.2.1 hace que el comodín *establezca* el nombre, así que una consulta por un tipo que el comodín no lleva es NODATA y no NXDOMAIN — negarlo de plano contradiría la respuesta positiva que ese mismo comodín da una consulta antes. Bajo DNSSEC el NXDOMAIN no solo está mal, es indemostrable: esa prueba necesita un NSEC que cubra `*.<parent>`, y ningún registro así puede existir mientras exista el comodín, de modo que la respuesta saldría con un registro de menos y se rechazaría. La prueba del NODATA es el NSEC **del propio comodín** (`Database::wildcard_owner_for` devuelve el propietario para que la ruta de denegación lo encuentre), porque el nombre consultado no está literalmente en la cadena y no tiene ninguno propio. Un comodín casa una etiqueta, así que a un nombre dos niveles por debajo no lo establece nada y se queda en NXDOMAIN.

- **A un TLD propiedad de otro ámbito de red se le niega cuanto exista en él.** Este es el único sitio donde el servidor devuelve NXDOMAIN por un nombre que conoce, y es la partición del horizonte partido funcionando según lo diseñado: NODATA afirmaría que el nombre es real y que solo falta el tipo, que es exactamente lo que permite a un ámbito enumerar los nombres de una red hermana observando cuáles responden NODATA en vez de NXDOMAIN. Se cuenta bajo `scope_hidden`.

`tests/nodata_test.rs` ejercita cada uno de estos casos sobre sockets UDP y TCP reales tras mutar a través del plano de control gRPC; la decisión en sí queda fijada por pruebas unitarias en `src/dns_server.rs`. Cada caso va emparejado con su control — un nombre sin nada en él ni por debajo, preguntado en la misma zona por la misma ruta — porque un servidor que respondiera NODATA a todo satisfaría solo la mitad positiva, afirmando de paso que todo nombre de la zona existe.
...
| 2 | local | Reenviador en claro que apunta a una dirección **privada** (el resolutor local/DHCP, la pasarela) |
| 3 | public | Reenviador en claro que apunta a una dirección **pública**, como último recurso |

La cadena existe para que la resolución sobreviva a redes que filtran el `:53` saliente. DoH se prefiere a DoT porque `:443` parece HTTPS corriente y sobrevive al DPI que deja pasar la conexión TCP de DoT pero descarta la sesión TLS. Los upstreams cifrados se marcan **por IP** y el certificado TLS se valida contra el nombre configurado, así que el escalón no necesita ningún DNS previo; el tiempo de espera por upstream es de 1,5 s.

**La pertenencia a un escalón se deriva del reenviador, no de la clave de configuración bajo la que se escribió.** Todo upstream — `forwarders`, `resolution.secure_upstreams`, `resolution.public_fallback` — se analiza y se convierte en un mismo `Forwarder` (`src/forwarder.rs`), y `by_preference` lo archiva por lo que *es*: cifrado, en claro privado o en claro público. Una entrada DoH escrita en `forwarders:` es un upstream cifrado y se prueba la primera; una entrada de `public_fallback` que apunta a la LAN es un reenviador local. Esto es lo que hace que los transportes sean alcanzables en tiempo de ejecución: `secure_upstreams` no tiene setter gRPC, así que antes de la unificación el único escalón que funcionaba en una red con el `:53` filtrado era también el único que nada podía reconfigurar sin reiniciar, mientras que el escalón que *sí* era programable era el que no podía responder.
...
La autoridad `name@ip` lleva en una sola cadena las dos mitades que necesita un upstream cifrado — la dirección a la que marcar y el nombre contra el que validar su certificado —, que es lo que permite que la lista entera siga siendo una lista de cadenas y conserva la propiedad de arranque de que no haya que resolver ningún nombre de host antes de poder alcanzar un resolutor. Omitir `name@` valida contra los propios SAN de IP de la dirección, lo cual es una comprobación real y no una renuncia. **DoQ pone a cero el ID del mensaje DNS en el cable** (RFC 9250 §4.2.1 lo exige; los flujos QUIC ya correlacionan la respuesta) y restaura el ID del llamante antes de devolverla, porque todo lo que hay por encima empareja respuestas con consultas por ese ID.

- **Un reenviador que sigue fallando deja de probarse.** Cada uno lleva un cortacircuitos (`forwarder::Health`): tres fallos de *transporte* consecutivos lo abren, y sigue abierto 30 s antes de dejar pasar una consulta para ver si se ha recuperado. Un éxito lo cierra de golpe. Esto es lo que abarata un escalón muerto — `auto` recorre la cadena entera antes de poder responder SERVFAIL, así que en una red que descarta un transporte por completo cada consulta pagaba el tiempo de espera íntegro de 1,5 s por cada dirección de dos escalones que nunca iban a responder: seis segundos de un fallo de siete segundos y medio, gastados en volver a establecer lo que las tres consultas anteriores ya habían demostrado. Los saltos se cuentan en `upstream_skipped_total{server}`.
- **La salud se registra desde el transporte, no desde el rcode.** Un SERVFAIL que llegó es un reenviador que funciona respondiendo mal a una pregunta; abrir el cortacircuitos por eso expulsaría a un resolutor sano por una sola zona rota. Solo cuenta en su contra no obtener respuesta alguna.
...
- **Los tipos no firmables se saltan, no se aproximan.** NSEC3, NSEC3PARAM y ANAME no tienen aquí formato de cable almacenado, y un valor mal formado no tiene codificación canónica; esos RRset se saltan y se nombran en el mensaje de respuesta. Una firma calculada sobre una codificación inventada es peor que ninguna — falla en cerrado en todos los validadores en vez de dejar el nombre sin firmar. NSEC ya no está en esa lista: tiene forma almacenada y codificador canónico, porque los registros de denegación de la propia zona hay que firmarlos como cualquier otra cosa.
- **La cadena NSEC se reconstruye en la misma pasada**, antes del bucle de firma, de modo que sus registros se firman junto con todo lo demás. Escritos después serían registros de denegación sin firmar, que es peor que ninguno: la zona anuncia DNSKEY prometiendo que toda respuesta va firmada, así que un validador lee una firma ausente como un ataque y no como una zona sin firmar. Véase [Generación de la cadena NSEC](#generación-de-la-cadena-nsec).
- **La refirma reemplaza.** Los RRSIG existentes en la zona se limpian primero, incluidos los de nombres cuyos registros se borraron desde la última pasada, así que las firmas nunca se acumulan ni sobreviven a sus datos. La cadena NSEC vieja se va con ellos — un eslabón que sobreviva en un nombre que la cadena nueva no cubre es una afirmación firmada de que los nombres de su rango no existen. El RRset DNSKEY se vuelve a publicar igualmente en vez de añadirse. La caché de respuestas se vacía después.
- **`SignZone` es uno de dos llamantes.** La operación vive en `src/zone_signer.rs` porque el bucle de refirma la llama también; véase [Mantener firmada una zona firmada](#mantener-firmada-una-zona-firmada).
...
DNSKEY, DS, RRSIG y NSEC se sirven bajo sus propios códigos de tipo, con el RDATA codificado por el mismo codificador canónico que hashea el firmante — así que lo que va por el cable es byte a byte lo que se firmó. URI y ZONEMD se codifican igual. (Antes se servían como registros TXT con la cadena almacenada, lo que responde a una consulta DNSKEY con un TXT y hace inservible cualquier firma publicada.) NSEC pasa por ese mismo codificador en vez de reconstruirse como un registro nativo de hickory, porque dos codificadores sobre un mismo registro es justo la discrepancia que este montaje existe para descartar. NSEC3 y NSEC3PARAM no se generan nunca y no se sirven: la denegación de existencia aquí es NSEC a secas.

### Generación de la cadena NSEC

Firmar una zona reconstruye su cadena NSEC a partir de los registros que guarda (`dnssec::build_nsec_chain`). Un NSEC por cada nombre que existe, en orden canónico, cada uno apuntando al siguiente, y el último dando la vuelta hasta el ápice para que los rangos tapicen el espacio de nombres entero sin dejar hueco.

- **El orden canónico no es el orden de cadena.** El RFC 4034 §6.1 compara las secuencias de etiquetas de derecha a izquierda, así que `example.com.` precede a `a.example.com.` mientras que una ordenación byte a byte los pone al revés. `dnssec::canonical_name_cmp` lo implementa. Equivocarse aquí falla en silencio: la cadena sigue existiendo, sencillamente sus rangos no tapizan la zona, y un validador no encuentra registro alguno que cubra el nombre por el que preguntó y llama bogus a una denegación correcta.
- **Los no-terminales vacíos existen y se encadenan.** Una zona que solo guarda `_443._tcp.host.example.com.` contiene también `_tcp.host.example.com.` y `host.example.com.` como nombres, sin registros propios. El RFC 4035 §2.3 les da registros NSEC igualmente; omitirlos deja sus nombres dentro del rango de algún otro eslabón, que es una afirmación firmada de que no existen — contradiciendo a la hija que demuestra que sí.
- **Todo mapa de bits declara NSEC y RRSIG**, porque firmar la cadena pone ambos en cada nombre que cubre. Un mapa de bits que los omitiera negaría los mismísimos registros que llevan la denegación.
- **El mapa de bits es el del RFC 4034 §4.1.2**: bloques de ventana en orden ascendente, cada uno recortado hasta su byte más alto con bits puestos, siendo el bit 0 el bit *alto* de un byte. Las ventanas se ordenan en vez de emitirse en el orden de inserción — un validador reconstruye el mapa de bits por su cuenta y compara bytes, así que «mismo conjunto, orden distinto» es un fallo de verificación.
- **Los valores NSEC se almacenan como `"<next_owner> <TYPE> <TYPE> ..."`**, los tipos por mnemónico y en orden ascendente de código de cable. Esa ordenación es lo que hace que una refirma sobre una zona sin cambios produzca una cadena byte a byte idéntica; sin ella, cada pasada reescribiría todos los RDATA NSEC e invalidaría todas las firmas NSEC sin motivo alguno.
- **El TTL de los NSEC es el MINIMUM del SOA**, acotado por el propio TTL del SOA exactamente como el RFC 2308 §3 acota un TTL negativo. Un solo mando gobierna cuánto tiempo se cachean las ausencias de esta zona, lleguen como un rcode pelado o como una prueba firmada.

### Servicio de DNSSEC

Firmar produce registros; servir es lo que los pone delante de un cliente, y son dos asuntos distintos que estaban incompletos por separado. La firma guardaba filas RRSIG que ninguna ruta de consulta llegaba a leer, así que una zona firmada respondía desnuda a toda consulta positiva y con un rcode pelado a toda negativa — y un resolutor validador lee una firma ausente en una zona que publica DNSKEY como **arrancada**, no como sin firmar. La zona era bogus en todas sus respuestas mientras todas las pruebas de firma seguían en verde.

Ahora se sirven ambas mitades, y ambas están condicionadas a lo mismo: que el cliente haya puesto el bit DO y que el nombre caiga dentro de una zona de la que este servidor tiene claves (`Database::find_signed_zone`, una búsqueda de sufijo en memoria en vez de una consulta contra la tabla `dnssec_keys`, que no está indexada). Ninguna de las dos es opcional — una firma que nadie pidió triplica aproximadamente el tamaño de una respuesta A, y una respuesta grande a una pregunta pequeña es la forma de amplificación que los CIDR de recursión existen para cerrar.

- **Las respuestas positivas** llevan los RRSIG que cubren el RRset, en la sección de respuesta junto a él (RFC 4035 §3.1.1). El tipo cubierto vive dentro del valor almacenado del RRSIG y no en una columna, así que `Database::rrsigs_covering` lee los RRSIG que hay en el propietario y filtra en Rust; la cuenta en un propietario es el número de RRset distintos que hay allí, que en cualquier zona real es de un solo dígito.
- **NODATA** lleva el NSEC del propio nombre consultado: su mapa de bits de tipos enumera lo que está presente, y la ausencia del tipo consultado de esa lista es la denegación.
- **NXDOMAIN** lleva dos pruebas — el eslabón de la cadena cuyo rango cubre el nombre, y el eslabón que cubre `*.<closest encloser>`. Sin la segunda, un validador no puede descartar que un comodín hubiera sintetizado una respuesta, y el RFC 4035 §5.4 hace que rechace la denegación. Con frecuencia las dos son el mismo registro, así que el resultado se deduplica por propietario en vez de concatenarse. El envolvente más cercano es el ancestro más largo del nombre consultado que existe (RFC 4592 §3.3.1), hallado subiendo con `name_occupied`.
- **El SOA también se firma.** Un validador autentica el TTL negativo desde la misma sección en la que lo lee, y un SOA sin firmar junto a un NSEC firmado es una sección de autoridad mixta que falla como un todo.
- **Las respuestas positivas derivadas de comodín no llevan firma**, deliberadamente. Sus registros se guardan en `*.parent` y se reescriben al nombre consultado, así que el RRSIG que los cubre está en el propietario del comodín y demostrar la síntesis exige un NSEC que enseñe que no existía ninguna coincidencia más cercana. Servir el RRset sin firmar deja a un validador en *insecure*; servirlo con una firma cuyo propietario no casa lo deja en *bogus*, que es estrictamente peor. La búsqueda es por nombre de propietario exacto, así que esto sale solo en vez de tratarse como caso especial.
- **Los registros con ámbito no se firman ni se demuestran nunca.** La firma opera sobre la tabla global, así que la ruta de denegación se aplica solo al espacio de nombres global; una negativa con ámbito lleva su SOA y nada más.

### Mantener firmada una zona firmada

**Firmar es una instantánea, y una zona no se mantiene firmada sola.** Un registro añadido después de `SignZone` no tiene RRSIG —que un validador lee como una firma arrancada y no como un nombre sin firmar— y, mucho peor, la cadena NSEC sigue describiendo la zona tal como era: el nombre nuevo cae dentro del rango de un eslabón existente, así que la zona sirve una *prueba firmada de que su propio registro no existe*. Nada de eso es visible para quien añadió el registro. Las firmas caducan además a 30 días, así que hasta una zona intacta se vuelve bogus por reloj.

Tres piezas cierran eso:

1. **Toda mutación de registros marca su zona.** `add_record` y `remove_records` ponen una bandera de refirma pendiente en la zona firmada que contiene el nombre. Las escrituras de DNSKEY, NSEC y RRSIG no lo hacen — esos son los productos de la firma, y contarlos dejaría al bucle ensuciando la zona que acababa de limpiar.
2. **Una cadena obsoleta se retiene, no se sirve.** Mientras una zona está marcada, las negativas salen sin su prueba. Eso deja a un validador en *insecure*, donde servir una cadena anterior a la mutación lo dejaría en *bogus* y tumbaría la zona entera. Lo cuenta `rolodex_dns_denial_proofs_withheld_total`, que es la métrica que dice que una zona firmada está respondiendo ahora mismo sin demostrar.
3. **Un bucle de refirma drena las marcas** cada 30 segundos (`DnsServer::resign_loop`, siempre lanzado, y sin efecto alguno si no hay zonas firmadas). Está fuera de la ruta de mutación porque firmar es O(zona) y Town OS inscribe registros a ráfagas en el arranque — refirmar de forma síncrona pagaría ese coste una vez por registro en lugar de una vez por ráfaga. Una pasada que falla deja la marca puesta para que la siguiente lo reintente, con las pruebas retenidas mientras tanto.
...
`authoritative_nxdomain` y `authoritative_nodata` se cuentan por separado porque hacen afirmaciones opuestas sobre el nombre consultado (véase [Respuestas negativas](#respuestas-negativas)), y solo una de ellas permite a un resolutor concluir algo sobre los nombres que hay por debajo. `rolodex_dns_authoritative_negative_total{reason}` dice entonces qué encontró la búsqueda: `name_absent` (nada en el nombre ni por debajo de él — un NXDOMAIN de verdad), `type_absent` (el nombre existe, el tipo no — NODATA), `unsupported_type` (el tipo de consulta no se corresponde con ninguna clase de registro almacenable, así que nada de él puede existir aquí) y `scope_hidden` (el nombre cae bajo un TLD propiedad de otro ámbito de red y se retiene). La familia de razones existe porque la etiqueta de origen de la respuesta dice qué etapa produjo una negativa, pero no qué vio, y esas son preguntas operativas distintas: que suba `unsupported_type` significa un tipo de registro que el servidor debería admitir, mientras que si sube `name_absent` significa clientes pidiendo nombres que no tiene nadie.
...
| `tests/forwarder_transport_test.rs` | Las dos propiedades para las que existe tipar un reenviador, cada una con su control: un ascendente cifrado escrito en la lista de texto plano llega al nivel **seguro** (una colocación que la forma anterior no podía expresar siquiera), y un reenviador agujereado deja de ser *marcado* — comprobado como un recuento de conexiones contra un socket de loopback que nunca responde, ya que una prueba de que acaba dando SERVFAIL pasa con o sin el cortacircuitos. |
...
Las pruebas unitarias de `src/dnssec.rs` cubren la propia forma canónica: paso a minúsculas y cualificación de nombres, recuentos de etiquetas del RFC 4034 §3.1.3, troceado de cadenas de caracteres, codificación de RDATA por tipo, independencia del orden del RRset, ida y vuelta de firma/verificación por algoritmo, detección de manipulación, negativa a cargar material de clave que contradice su etiqueta, y que los nombres de algoritmo almacenados hacen ida y vuelta por `parse`. Las incorporaciones de NSEC quedan fijadas ahí también, y cada expectativa está escrita a mano contra el RFC en vez de compararse con el codificador que la produjo — un mapa de bits comprobado contra su propio generador solo demuestra que el generador es determinista, mientras que un validador reconstruye esos bytes por su cuenta. Eso cubre el orden de bits del §4.1.2 (el bit 0 es el bit *alto*), el ejemplo resuelto del §4.3, la omisión de ventanas vacías y el recorte por ventana, la ordenación canónica de nombres **con su control de que el orden de cadena a secas discrepa**, la vuelta de la cadena, la cobertura de los no-terminales vacíos, la declaración de NSEC/RRSIG en todo mapa de bits, la ida y vuelta de la forma almacenada en orden de código de cable, el rechazo de un mnemónico desconocido y — la propiedad sobre la que gira el diseño entero — que la cadena generada cubre cada nombre ausente exactamente una vez y ningún nombre presente.

### Pruebas de servicio DNSSEC

`tests/dnssec_serving_test.rs` es la otra mitad de la batería de firma: si algo de ella llega al cable. La firma guardaba filas RRSIG que ninguna ruta de consulta leía, así que una zona firmada respondía desnuda a toda consulta positiva y con un rcode pelado a toda negativa — bogus en todos los validadores mientras todas las pruebas de firma seguían en verde, que es justo el hueco que una batería a nivel de base de datos no puede ver.

La invariante es que una respuesta sea **comprobable por quien la recibe**, así que las pruebas re-derivan la verificación a partir del DNSKEY publicado y contrastan los rangos NSEC contra el nombre por el que se preguntó de verdad — las dos cosas que hace un validador. Afirmar que un RRSIG o un NSEC meramente apareció pasaría igual con una firma sobre los bytes equivocados o con una prueba que cubre el intervalo equivocado, y ambas fallan en un resolutor y no aquí.

El fichero se sostiene sobre tres parejas de control: un cliente con DO frente a uno sin DO (un servidor que firmara todo sin distinción pasaría de otro modo, y eso no es inofensivo — una respuesta A firmada es aproximadamente tres veces el tamaño de una desnuda), una zona firmada frente a una sin firmar (sin la cual toda afirmación la cumpliría igualmente un servidor que emitiera registros DNSSEC indiscriminadamente), y una prueba servida frente a una prueba retenida a propósito. Cubre una firma verificable llegando a un cliente con DO y no llegando a nadie más, NODATA demostrado por el mapa de bits de tipos (afirmando que el mapa declara el tipo que *sí* está presente y no el que la respuesta niega), NXDOMAIN llevando tanto un NSEC que lo cubre como la denegación del comodín, con los rangos re-derivados en vez de dados por supuestos, el no-terminal vacío estando en la cadena y respondiendo NODATA, una mutación posterior a la firma reteniendo la cadena obsoleta mientras el registro nuevo sigue respondiendo, la pasada de refirma restaurando las pruebas y cubriendo el nombre nuevo con una firma verificable, y la firma no ensuciando la zona que acaba de firmar — sin lo cual el bucle de refirma no se asentaría nunca.

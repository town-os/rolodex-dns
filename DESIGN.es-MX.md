...
- **Pruebas de documentación** (`tests/promql_docs_test.rs`): se analiza cada bloque ```promql de `README.md` y `DESIGN.md`, se extraen sus nombres de métrica y emparejadores de etiqueta, y cada uno se resuelve contra la salida de exposición en vivo. La documentación es la única parte de un cambio de métricas que nada más verifica — renombrar `{type}` a `{message_type}` deja el código compilando y todas las demás pruebas en verde mientras convierte en silencio cada consulta documentada de cuadro de mando en una que no devuelve datos, y el operador se entera cuando un panel se queda en blanco a mitad de incidencia. El mismo archivo fija el **recuento de familias** documentado contra lo que emite `render` (ya había derivado, 73 documentadas contra 74 emitidas) y protege la propia valla, porque un bloque reetiquetado como `  1  ` haría que el archivo entero dejara de revisar nada. El analizador está hecho a mano en vez de arrastrar `regex`, y es deliberadamente permisivo con la sintaxis que no entiende y estricto con los identificadores que sí.

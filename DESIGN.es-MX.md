...
| 2 | local | Do53 en claro hacia los `forwarders` configurados (el resolvedor local/DHCP) |
...
- **La refirma reemplaza.** Los RRSIG existentes en la zona se limpian primero, incluidos los de nombres cuyos registros se borraron desde la última pasada, así que las firmas nunca se acumulan ni sobreviven a sus datos. El RRset DNSKEY se vuelve a publicar igualmente en vez de agregarse. El caché de respuestas se vacía después.

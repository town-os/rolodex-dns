...
`tests/nodata_test.rs` ejercita cada uno de estos casos sobre sockets UDP y TCP reales tras mutar por el plano de control gRPC; la decisión en sí la fijan pruebas unitarias en `src/dns_server.rs`. Cada caso va emparejado con su control — un nombre sin nada en él ni por debajo, preguntado en la misma zona y por la misma ruta — porque un servidor que respondiera NODATA a todo satisfaría solo la mitad positiva mientras afirma que todos los nombres de la zona existen.
...
Los gestores que sirven material **generado** no se sondean en absoluto: no hay archivo detrás de ellos, y regenerar con un temporizador le entregaría a cada cliente un certificado autofirmado distinto dos veces por minuto, lo que es indistinguible de un ataque para cualquier cosa que hubiera fijado el anterior. `src/main.rs` retiene todos los gestores durante la vida del proceso; un administrador descartado es un emisor de watch descartado, y a sus escuchas no se les podría volver a entregar nada.
...
| `dnssec.trust_anchors`              | `[]` (llaves raíz de IANA)     | Anclas de confianza como `"<flags> <protocol> <algorithm> <clave base64>"` (los campos RDATA de un DNSKEY, tal como los imprime `dig`); cada campo se valida en el arranque y uno malo es un fallo duro. Una sustitución **reemplaza** las llaves de IANA en vez de sumarse a ellas |

...
Construido con Axum y axum-server para el soporte TLS. El transporte HTTP/3 (QUIC) se puede activar con la bandera de configuración `enable_h3`.
...
| `acme.tlsa_endpoints`              | `[]`                           | Endpoints `"<puerto>/<protocolo>"` extra para el registro DANE-TA. Un certificado que sirve DoT (`853/tcp`) y DoQ (`853/udp`) necesita un registro por endpoint; una entrada malformada detiene el arranque |
...
Cuatro baterías cubren superficies que antes tenían solo una prueba de humo de compilación o una prueba unitaria de análisis de configuración:

...
| `acme.tlsa_endpoints`              | `[]`                           | Extra `"<port>/<proto>"` endpoints for the DANE-TA record. One certificate serving DoT (`853/tcp`) and DoQ (`853/udp`) needs one record per endpoint; a malformed entry stops startup |

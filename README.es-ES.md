...
**DNS-over-HTTPS (DoH)** — RFC 8484, puerto 443 por defecto. Consultas DNS sobre HTTPS con soporte de los métodos GET (`/dns-query?dns=<base64>`) y POST (`application/dns-message`). Opcionalmente admite HTTP/3 por QUIC (`enable_h3: true`). Se configura con la sección `doh` en YAML o con `SetDohConfig` por gRPC.

...
> Idiomas: [English](README.md) | [繁體中文](README.zh-TW.md) | [简体中文](README.zh-CN.md) | **Español (España)** | [Español (México)](README.es-MX.md) | [日本語](README.ja-JP.md)

Un servidor DNS de horizonte partido y resolutor recursivo/reenviador con la privacidad por delante, con transportes cifrados, DNSSEC y gestión por gRPC, escrito en Rust.

Rolodex DNS ofrece DNS sobre UDP, TCP, TLS (DoT), HTTPS (DoH) y QUIC (DoQ) con una base de datos local de registros que tiene prioridad sobre la resolución externa. Los registros se gestionan en remoto por gRPC (autenticación por secreto compartido sobre TCP, o sin autenticar por socket Unix). Admite resolución a nivel de TLD con superposición de dominios, de modo que las representaciones internas del DNS siempre se prefieren. Una caché de respuestas DNS integrada evita la fuga de consultas a los resolutores upstream una vez que un registro se ha visto.

Los nombres que no son locales se resuelven **iterativamente desde los servidores raíz** por defecto, cayendo por upstreams cifrados (DoH/DoT) y en claro, de modo que la resolución sobrevive a redes que filtran el puerto 53 saliente. Véase [Resolución upstream](#resolución-upstream).

Las respuestas resueltas desde las raíces se **validan con DNSSEC** contra las anclas de confianza de IANA por defecto; los datos bogus no se sirven ni se cachean nunca. Véase [DNSSEC](#dnssec).
...
- **Resolutor recursivo con reserva resiliente**: resolución iterativa desde los servidores raíz por defecto, luego DoH/DoT a resolutores públicos, luego los reenviadores configurados, luego resolutores públicos en claro — de modo que la resolución sigue funcionando en redes que filtran `:53` (y con DPI que bloquea el `:853` de DoT). Un nivel pegajoso evita pagar tiempos de espera en un camino muerto, y todo cambio de nivel vacía la caché
- **Cacheado del resolutor que honra los TTL**: una caché persistente de delegaciones zona→servidor de nombres (caliente entre reinicios), una caché en memoria para glue, búsquedas de NS sin glue y saltos CNAME, y cacheado negativo del RFC 2308 — todo servido con su vida restante
...
- **Resolutor reenviador**: reenviadores DNS upstream configurables, usables en exclusiva con `resolution.mode: forward`
- **Superposición de TLD/dominios**: añade registros en cualquier nivel (incluidos TLD) para sustituir el DNS público
- **Firma DNSSEC**: generación de claves Ed25519 (preferido) y ECDSA P-256/P-384, firma de zonas, cálculo de registros DS y una cadena NSEC para la denegación autenticada. RSA/SHA-256 se verifica pero no se puede generar (`ring` no genera claves RSA). Las firmas y las pruebas se sirven a los clientes que ponen DO, y un bucle de refirmado mantiene firmada una zona mutada en lugar de dejarlo a un `sign-zone` manual
- **Validación DNSSEC**: las respuestas resueltas iterativamente se validan contra las anclas de confianza de la raíz de IANA, activo por defecto (`dnssec.validate`). La cadena se construye de arriba abajo junto al recorrido de delegaciones, así que un DS no cuesta ninguna consulta extra; una delegación sin firmar debe *demostrar* que lo está (NSEC/NSEC3 firmado), de modo que arrancar firmas no es una degradación. Los datos bogus son SERVFAIL y no se cachean nunca, y AD se pone solo para respuestas genuinamente Secure
...
- **DNS64 (RFC 6147)**: síntesis AAAA a partir de registros A con prefijo configurable (por defecto `64:ff9b::/96`)
...
- **Gestión por gRPC**: gestión remota de registros por gRPC con autenticación por secreto compartido o por socket Unix
- **Listas de bloqueo**: comprobación de proveedores DNSBL con cacheado en memoria, más una base de datos local de bloqueo para entradas propias
- **Soporte de DNSBL**: listas de bloqueo de dominios (Spamhaus DBL, SURBL, URIBL) comprobadas antes de cualquier resolución externa, así que un nombre listado se rechaza incluso si antes se había cacheado una respuesta reenviada
- **Manejo de rechazos de lista de bloqueo**: una DNSxL responde «listado» y «deja de consultarnos» con la misma clase de registro `A`, así que los códigos de rechazo (`127.255.255.254`, `127.0.0.1`, …) se reconocen como *no* siendo un listado y el proveedor se saca de la rotación de consultas durante un enfriamiento — en vez de dar NXDOMAIN a todos los nombres comprobados contra él
- **Lista de permitidos de la lista de bloqueo**: una única salida de emergencia que cubre todas las listas y ambas compuertas — una entrada exime a un nombre y a sus subdominios de la comprobación DNSBL/local, y a una dirección (por nombre inverso o literal IP) de la comprobación de búsqueda inversa
- **Control de acceso a la recursión**: `security.recursion_cidrs` decide quién puede dirigir la resolución *upstream*, y por defecto trae los rangos no enrutables desde internet, así que un enlace por defecto a `0.0.0.0:53` no es un resolutor recursivo abierto. Los desconocidos siguen recibiendo las respuestas autoritativas de este servidor
...
- **Servidor DHCPv4 integrado**: conjuntos de direcciones por ámbito con vinculaciones MAC pegajosas, registro automático de A/PTR, entrega de certificados mediante opciones específicas del sitio, y un barrido de concesiones en segundo plano
- **Registros PTR inversos automáticos**: mantenimiento opcional (`dns.auto_ptr`) de los PTR `in-addr.arpa`/`ip6.arpa` correspondientes a los registros A/AAAA añadidos por gRPC
- **Soporte de proxy**: reenvía las consultas DNS a través de un proxy HTTP CONNECT, SOCKS5 o DoH
- **Métricas de Prometheus**: un endpoint `/metrics` opcional y apagado por defecto que expone 85 familias de métricas con cardinalidad de etiquetas acotada — incluidas la atribución de respuestas por etapa y el aislamiento por TLD, de modo que la tubería de horizonte partido es legible desde fuera. Los nombres de consulta nunca son etiquetas
- **Persistencia en SQLite**: los registros DNS persisten entre reinicios
- **Recarga en caliente de TLS**: los ficheros de certificado se sondean cada 30 segundos y un par renovado lo sirven DoT, DoH, DoQ, ACME y el portal de inscripción dentro de esa ventana, sin reinicio y sin conexiones caídas. Una reconstrucción que falla —un fichero truncado, o un sondeo que cayó entre las dos escrituras de un cliente ACME— mantiene sirviendo el certificado anterior y reintenta en el siguiente sondeo
- **Rendimiento**: runtime tokio multihilo, estado de listas de bloqueo y del resolutor sin cerrojos (`AtomicBool` + `ArcSwap` + atómicos), cachés de arranque en memoria para ámbitos/zonas/TLD/entradas de bloqueo, pool de sockets UDP para el reenvío upstream, y cacheado concurrente con DashMap/DashSet por todas partes
...
Ejecuta el lint (la comprobación de deriva de las traducciones, `cargo fmt --check` + `clippy --all-targets -D warnings`), las pruebas de integración y unitarias de Go, las pruebas de integración y unitarias de Rust, las pruebas de lint/integración/unitarias de JavaScript, y la comprobación de ejecución del PromQL documentado. La capa de integración de Rust incluye baterías con sockets reales para la firma y la validación DNSSEC (contra una jerarquía simulada firmada cuyas respuestas se manipulan en el momento de serializar, así que cada prueba es «un despliegue válido, atacado»), el contrato NXDOMAIN de las listas de bloqueo, los códigos de rechazo de las listas de bloqueo, DoQ, el proxying, la recarga de TLS, ZONEMD, la administración de ACME, y una batería `security_*` por hallazgo de seguridad. Usa `make test-log` para la misma ejecución duplicada a un fichero de registro con marca de tiempo bajo `/tmp/rolodex-dns/log` (sustituible con `LOG_DIR`), impresa al final incluso si falla. Capas individuales: `make lint`, `make rust-test`, `make rust-integration-test`, `make go-test`, `make go-integration-test`, `make js-test`, `make js-integration-test`.

`make test` ejecuta además `make prometheus-test`, que pasa todas las consultas PromQL documentadas en este fichero por un contenedor de Prometheus real que raspa un servidor vivo — cazando una consulta mal formada *como PromQL* y no meramente una que nombre una serie inexistente. Necesita podman; sin él la comprobación **salta ruidosamente** en vez de fallar, así que una máquina sin runtime de contenedores sigue obteniendo una ejecución en verde sin fingir jamás que las consultas se verificaron. Pon `ROLODEX_PROMETHEUS_REQUIRED=1` para convertir ese salto en un fallo duro, y `ROLODEX_PROMETHEUS_IMAGE` para apuntar a un espejo de la imagen.
...
2. Arranca el servidor usando `dev.yml` con los siguientes ajustes:
   ...
   - Sin autenticación
   - Comprobación de listas de bloqueo desactivada
   - Reenviadores upstream por defecto (`8.8.8.8:53`, `8.8.4.4:53`), usados como el nivel `local` de la cadena de resolución `auto` por defecto

`make help` enumera todos los objetivos con una descripción, agrupados por sección (es además el objetivo por defecto, así que un `make` pelado lo imprime).
...
Una vez el servidor de desarrollo está en marcha, lo puedes gestionar con el binario `rolodex-dns-cli` o con la biblioteca cliente de Go conectada a `/tmp/rolodex-dns.sock`. Pulsa Ctrl+C para detener el servidor.
...
Las compilaciones son **nativas**: cada arquitectura se compila en un equipo de esa arquitectura. Toda imagen se etiqueta con un sufijo de arquitectura usando el nombre de máquina de `uname -m` (`-x86_64` o `-aarch64`, *no* los nombres OCI `amd64`/`arm64`), así que un equipo de despliegue puede bajar `` <etiqueta>-`uname -m` `` sin ningún mapeo. Un paso de manifiesto aparte ensambla las imágenes por arquitectura en una única etiqueta multiarquitectura.

#### Elegir la arquitectura: `TARGET`

`TARGET` selecciona la arquitectura para todos los objetivos de contenedor (`image`, `push-arch`, `push-rc`, `push-release`). Por defecto es la arquitectura del equipo, y coincide con el modelo `TARGET=` que usa el repo `install` de town-os, de modo que el mismo valor se puede pasar a cualquiera de los dos:
...
Los pasos RUN de `podman build` comparten la red del equipo (`--network=host`) para poder usar un resolutor DNS en el loopback del equipo (por ejemplo rolodex mismo); sustitúyelo con `BUILD_NETWORK=` para desactivarlo.
...
Ambas arquitecturas se compilan de forma cruzada en el equipo que ejecute `make`, usando `cargo-zigbuild` con zig como compilador cruzado de C y enlazador. `make deps` aprovisiona la cadena de herramientas entera **sin root**, y comprueba `python3` (que `make translation-check` necesita y que no puede instalar sin root):

```bash
make deps        # objetivos de rustup + cargo-zigbuild + zig, las dependencias de desarrollo de JS y una comprobación de python3
make cross-deps  # solo la cadena cruzada de Rust
```

Un `rustup target add` a secas no bastaría: `rusqlite` compila las fuentes C empaquetadas de SQLite y `ring` compila C y ensamblador, así que tiene que haber una cadena cruzada de **C** de verdad o la compilación falla en el paso `cc`. zig aporta una sin paquetes específicos de la distribución, y enlaza contra una glibc fijada (`GLIBC_VERSION`, por defecto `2.36` para coincidir con `debian:bookworm`), de modo que el binario corre sobre la imagen de ejecución sea cual sea la que lleve el equipo de compilación.
...
Las cachés del registro de Cargo y de git se persisten en `.cache/` para acelerar las recompilaciones.
...
Rolodex DNS lee la configuración de un fichero YAML (por defecto: `rolodex-dns.yml`, sustituible con `-c`/`--config`). Toda sección es opcional — si falta el fichero, el servidor arranca con los valores por defecto.
...
| `interfaz:puerto` | `eth0:53` | Enlazar a todas las IP de la interfaz de red nombrada |
...
El enlace por interfaz resuelve todas las direcciones IPv4 e IPv6 asignadas a la interfaz y crea un escucha aparte para cada una. Por ejemplo, si `eth0` tiene `192.168.1.5` y `fe80::1`, entonces `eth0:53` crea escuchas tanto en `192.168.1.5:53` como en `[fe80::1]:53`.
...
(el valor por defecto de Linux) un socket `[::]` acepta también tráfico v4 mapeado, así
que choca con un socket `0.0.0.0` en el mismo puerto y el segundo en enlazar falla con
...
# Reenviadores DNS upstream. Un dirección:puerto a secas es UDP en texto plano,
...
# no de esta clave: un reenviador cifrado aquí se prueba en el nivel seguro.
...
# Estrategia de resolución upstream (todos los campos opcionales; se muestran los valores por defecto)
resolution:
...
dnssec:
  ...
  trust_anchors: []       # vacío = las claves raíz de IANA; una sustitución las REEMPLAZA

# Cada entrada empareja un protocolo (udp/tcp) con una dirección de enlace.
# Las direcciones aceptan ip:puerto, [ipv6]:puerto, primary:puerto o interfaz:puerto.
dns:
  bind:
    - udp: "0.0.0.0:53"     # o "eth0:53" para enlazar a una interfaz concreta
    - tcp: "0.0.0.0:53"
  auto_ptr: false           # mantener PTR inversos para los A/AAAA añadidos por gRPC
...
dot:
  ...
  tls:
    ...
    # direcciones de enlace del propio escucha se cubren automáticamente; lista aquí
...
# Listas de bloqueo de dominios (comprobadas por nombre, antes de cualquier resolución externa)
dnsbl:
  # Activa/desactiva globalmente la comprobación de listas de bloqueo (por defecto: false)
  enabled: false
...
dns64:
  enabled: false
  prefix: "64:ff9b::"    # prefijo bien conocido por defecto (64:ff9b::/96)
...
| `dnssec.trust_anchors` | `[]` (claves raíz de IANA) | Anclas en forma de presentación DNSKEY, `"<flags> <protocolo> <algoritmo> <clave en base64>"` — los campos RDATA tal como los imprime `dig DNSKEY .`. Cada campo se valida al arrancar y uno incorrecto es un fallo duro. Una sustitución **reemplaza** las claves de IANA en vez de añadirse a ellas |
| `dns.bind` | `[{udp: "0.0.0.0:53"}, {tcp: "0.0.0.0:53"}]` | Escuchas DNS; lista de entradas `{udp: dirección}` / `{tcp: dirección}` |
| `dns.auto_ptr` | `false` | Mantiene registros PTR inversos para los A/AAAA añadidos por gRPC |
| `dns.ingress_listen_port` | `53` | Puerto UDP/TCP para las escuchas de ingreso por TLD (la IP de enlace es por TLD) |
| `dns.udp_shards` | `0` (uno por núcleo) | Sockets `SO_REUSEPORT` enlazados por cada dirección UDP de escucha. Un socket único serializa la escucha —un bucle de recepción, un socket para cada respuesta— y limita el rendimiento muy por debajo de la saturación de CPU. El fragmentado deja que el núcleo reparta los datagramas entre CPU. Pon `1` para el comportamiento antiguo de socket único |
...
| `dot.tls.key_path` | `""` | Ruta a la clave privada TLS para DoT |
| `dot.tls.auto_self_signed` | `true` | Genera automáticamente un certificado autofirmado para DoT |
| `dot.tls.self_signed_sans` | `[]` | Nombres alternativos del sujeto adicionales para un certificado DoT generado. El conjunto de loopback y las direcciones de enlace de la escucha se añaden automáticamente; un enlace comodín (`0.0.0.0`) no aporta nada, así que nombra aquí la máquina |
...
| `doh.tls.key_path` | `""` | Ruta a la clave privada TLS para DoH |
...
| `doq.tls.key_path` | `""` | Ruta a la clave privada TLS para DoQ |
...
| `dnsbl.enabled` | `false` | Activa globalmente la comprobación de listas de bloqueo de dominios (DNSBL) |
...
| `dnsbl.providers[].refusal_codes` | `[]` (conjunto integrado) | Respuestas que significan «consulta rechazada» en vez de «listado». Cada entrada es una dirección IPv4 o `dirección/prefijo`. Vacío significa el conjunto integrado; la entrada única `none` desactiva la detección para ese proveedor. Una lista explícita reemplaza los valores por defecto en vez de ampliarlos, y un código no interpretable se rechaza al arrancar (véase [Códigos de rechazo](#códigos-de-rechazo-y-rotación-de-proveedores)) |
| `dnsbl.providers[].refusal_cooldown_secs` | (por defecto de la lista) | Duración de la salida de rotación por proveedor tras un rechazo |
| `dnsbl.refusal_cooldown_secs` | `3600` | Segundos que un proveedor que rechaza permanece fuera de rotación, para los proveedores que no fijan ninguno. `0` significa «usa el valor por defecto», no «sin enfriamiento» |
| `dhcp.bind` | `"0.0.0.0:67"` | Escucha DHCP (sección ausente = DHCP desactivado) |
| `dhcp.tld` | -- | Obligatorio cuando DHCP está activo: los nombres de máquina se registran como `<host>.lan.<tld>.` |
| `dhcp.default_lease_duration` | `3600` | Duración de concesión por defecto en segundos |
| `dhcp.reclaim_timeout` | `86400` | Segundos tras la expiración antes de reclamar una IP |
...
| `acme.tlsa_endpoints` | `[]` | Endpoints `"<puerto>/<protocolo>"` adicionales en los que publicar el registro TLSA DANE-TA, más allá de `tlsa_port`/`tlsa_proto`. Un registro TLSA nombra un endpoint de servicio, así que un certificado que sirve DoT (`853/tcp`) y DoQ (`853/udp`) necesita un registro para cada uno; una entrada malformada se rechaza al arrancar en vez de saltarse. Un endpoint que un escucha sirve con sus propios ficheros de certificado se descarta al arrancar en vez de publicarse, y lo dice: la asociación de ACME fijaría un certificado que ese endpoint nunca presenta, y un cliente DANE que no casa ningún registro rechaza la conexión |
...
| `metrics.bind` | `127.0.0.1:9153` | Escucha HTTP `/metrics` de Prometheus; admite interfaz:puerto. La sección es opcional y se omite por defecto, en cuyo caso no se arranca escucha alguna (véase [Métricas de Prometheus](#métricas-de-prometheus)) |
...
Opciones:
  -c, --config <CONFIG>  Ruta al fichero de configuración [por defecto: rolodex-dns.yml]
  -h, --help             Muestra la ayuda
```

### Cliente CLI

`rolodex-dns-cli` es un cliente de línea de órdenes para gestionar un servidor Rolodex DNS en marcha a través de su interfaz de gestión gRPC. Admite tanto el transporte TCP como el de socket Unix.
...
|---------|-------------|
| **Registros** | |
| `add-record` | Añade un registro DNS a la base de datos local |
...
| `flush-cache` | Vacía la caché de resultados de las listas de bloqueo |
| `add-local-blocklist` | Añade una entrada a la lista de bloqueo local |
...
| `add-dnsbl-allow` | Exime a un nombre (y a sus subdominios) de la comprobación de listas de bloqueo |
...
| `add-scoped-record` | Añade un registro DNS dentro de un ámbito |
...
| `list-scope-tld-listeners` | Lista las escuchas DNS de ingreso enlazadas a los TLD de un ámbito |
| **Zonas autoritativas** | |
...
| **Caché** | |
| `cache-stats` | Muestra las estadísticas de aciertos/fallos de la caché DNS |
| `flush-dns-cache` | Vacía la caché de respuestas DNS |
| **DHCP** | |
| `add-dhcp-pool` / `remove-dhcp-pool` / `list-dhcp-pools` | Gestiona los conjuntos de direcciones DHCP por ámbito |
| `list-dhcp-leases` / `delete-dhcp-lease` | Inspecciona y elimina concesiones DHCP |
| `set-dhcp-cert` / `remove-dhcp-cert` / `list-dhcp-certs` | Gestiona la entrega de certificados mediante opciones DHCP |
| **DNSSEC** | |
| `generate-dnssec-key` | Genera un par de claves DNSSEC (KSK o ZSK) |
| `list-dnssec-keys` | Lista las claves DNSSEC de una zona |
| `sign-zone` | Firma una zona con sus claves DNSSEC |
| **DANE / ACME** | |
...
| `acme-status` | Comprueba el estado de un certificado ACME |
...
| `set-tracked-tlds` / `list-tracked-tlds` | Gestiona qué TLD reciben su propia etiqueta `tld` en las métricas de consultas por TLD |
...
Añade un registro DNS a la base de datos local.
...
| `--ttl <TTL>` | `300` | Tiempo de vida en segundos. Si se pone a 0, el servidor usa 300 por defecto |
...
| Opción | Por defecto | Descripción |
|--------|---------|-------------|
| `-f, --forwarders <ADDR>...` | -- | Especificaciones de reenviador upstream: un `"host:puerto"` a secas para UDP en texto plano, o una especificación con esquema para un upstream cifrado o TCP (véase [Especificaciones de reenviador](#especificaciones-de-reenviador)). Varias especificaciones separadas por espacios |
...
sin reiniciar. El `resolution.mode` del fichero de configuración es solo la semilla
...
`auto` como hace el fichero de configuración: a quien llama y escribe mal un modo no
se le puede decir que la máquina está en uno mientras resuelve en otro.

Ejemplos:
```bash
# Cadena de reserva con las raíces primero (el valor por defecto)
...
primeras consultas tras el cambio no pagan el coste del nivel frío.

##### `get-resolution-mode`

Muestra el modo actualmente en vigor. Es el modo que de verdad está resolviendo
consultas, que no tiene por qué ser el que nombra el fichero de configuración — los
dos difieren tras un `set-resolution-mode`.
...
Vacía la caché de resultados de las listas de bloqueo. Fuerza búsquedas frescas para las consultas siguientes.
...
| `-d, --home-domain <DOMAIN>` | `"<name>.home."` | Dominio `.home` reservado para este ámbito. Si se omite, por defecto es `"<name>.home."` |

Ejemplos:
```bash
# Crea un ámbito con el dominio home por defecto
...
| `--ttl <TTL>` | `300` | TTL en segundos de la asociación. Debe refrescarse antes de que expire. Si es 0, por defecto son 300 |

Ejemplos:
```bash
# Se une con el TTL por defecto
...
Añade un registro DNS dentro de un ámbito de red concreto. Los registros con ámbito solo son visibles para las IP asociadas a ese ámbito.
...
| Opción | Por defecto | Descripción |
|--------|---------|-------------|
| `-s, --scope <SCOPE>` | -- | Ámbito de red al que añadir el registro |
...
La API de gestión está definida en `proto/rolodex_dns.proto`. Todos los métodos aceptan un campo `auth_token` para la autenticación por secreto compartido al conectar por TCP. Las conexiones por socket Unix se saltan la autenticación.

Consulta el fichero proto para la referencia completa de la API. El servicio define 74 métodos RPC que cubren la gestión de registros, los ámbitos de red, los TLD propios y el ingreso, las listas de bloqueo, DHCP, los transportes cifrados, DNSSEC, DANE/ACME, el cacheado, DNS64, las métricas y la observabilidad.
...
**Parámetros:**
- `record` (DnsRecord, obligatorio): el registro DNS a añadir
  ...
  - `value` (string): datos del registro (p. ej. dirección IP, nombre de máquina)
  - `ttl` (uint32): tiempo de vida en segundos. Por defecto: 300 si se pone a 0
  - `priority` (uint32): prioridad para los registros MX/SRV (se ignora en los demás tipos). Por defecto: 0
...
- `filter_by_type` (bool): si se aplica el `record_type_filter`. Por defecto: falso
- `auth_token` (string): secreto compartido para la autenticación
...
**Parámetros:**
- `forwarders` (repeated string): especificaciones de reenviador upstream. Un `"host:puerto"` a secas es UDP en texto plano (p. ej. `"8.8.8.8:53"`); un esquema selecciona el transporte — `"tcp://8.8.8.8:53"`, `"tls://cloudflare-dns.com@1.1.1.1:853"`, `"https://cloudflare-dns.com@1.1.1.1/dns-query"`, `"quic://dns.adguard.com@94.140.14.14:853"` (véase [Especificaciones de reenviador](#especificaciones-de-reenviador)). La lista entera se analiza antes de aplicar nada de ella, y una especificación mal formada hace fallar la llamada con `INVALID_ARGUMENT`: una lista instalada a medias porque su cuarta entrada tenía una errata deja el resolutor en un estado que quien llamó ni pidió ni puede ver
- `auth_token` (string): secreto compartido para la autenticación
...
cambiar sin reescribir ese fichero y reiniciar el proceso — y reiniciar el único
resolutor de una máquina es una caída de DNS para todo lo que hay en ella.
...
el camino del fichero de configuración. Un fichero lo lee una vez al arrancar alguien
...
de arranque, así que las primeras consultas tras el cambio no pagan el coste del nivel
...
resolviendo consultas, no el que nombra el fichero de configuración. Los dos difieren
tras una llamada a `SetResolutionMode`.
...
Vacía la caché de búsquedas de las listas de bloqueo.
...
- `scope` (NetworkScope, obligatorio): el ámbito a crear
  ...
  - `home_domain` (string): dominio `.home` reservado. Por defecto: `"<name>.home."` si está vacío
- `auth_token` (string): secreto compartido para la autenticación
...
- `ttl_seconds` (uint64): TTL en segundos. Por defecto: 300 si se pone a 0. Debe refrescarse antes de que expire.
- `auth_token` (string): secreto compartido para la autenticación
...
**Parámetros:**
- `scope_name` (string, obligatorio): el ámbito al que añadir el registro
- `record` (DnsRecord, obligatorio): el registro DNS a añadir
- `auth_token` (string): secreto compartido para la autenticación
...
| `GetCacheStats` | Obtiene las estadísticas de la caché DNS (entradas, aciertos, fallos) |
| `FlushDnsCache` | Vacía la caché de respuestas DNS |
...
| `AddLocalBlocklistEntry` | Añade una entrada a la lista de bloqueo local |
...
| `AddDnsblAllowlistEntry` | Exime a un nombre (y a sus subdominios) de la comprobación de listas de bloqueo |
...
| `SetScopeTldForwarders` / `ListScopeTldForwarders` | Gestiona los reenviadores pares de un TLD |
| `ListScopeTldListeners` | Lista las escuchas DNS de ingreso enlazadas a los TLD de un ámbito |
| `AddDhcpPool` / `RemoveDhcpPool` / `ListDhcpPools` | Gestiona los conjuntos de direcciones DHCP por ámbito |
| `ListDhcpLeases` / `DeleteDhcpLease` | Inspecciona y elimina concesiones DHCP |
| `SetDhcpCertOption` / `RemoveDhcpCertOption` / `ListDhcpCertOptions` | Gestiona la entrega de certificados mediante opciones DHCP |
...
| `GenerateDnssecKey` | Genera un par de claves DNSSEC para una zona |
| `ListDnssecKeys` | Lista las claves DNSSEC de una zona |
| `DeleteDnssecKey` | Elimina una clave DNSSEC |
| `GetDsRecords` | Obtiene los registros DS para la delegación desde la zona padre |
| `SignZone` | Firma (o vuelve a firmar) una zona con sus claves DNSSEC |
...
| 15 | `DNSKEY` | Clave pública DNSSEC. Gestionada automáticamente por la generación de claves DNSSEC |
| 16 | `DS` | Firmante de delegación. Gestionado automáticamente por DNSSEC |
| 17 | `RRSIG` | Firma de registro de recurso DNSSEC. Gestionada automáticamente por la firma de zonas |
| 18 | `NSEC` | Registro «siguiente seguro» (DNSSEC). Gestionado automáticamente por la firma de zonas |
| 19 | `NSEC3` | Registro «siguiente seguro» v3 (DNSSEC). Gestionado automáticamente por la firma de zonas |
| 20 | `NSEC3PARAM` | Parámetros de NSEC3 (DNSSEC). Gestionados automáticamente por la firma de zonas |
...
| 22 | `SVCB` | Vinculación de servicio (RFC 9460). El valor es una línea de formato de presentación: `"<prioridad> <destino> [clave=valor ...]"` — p. ej. `"1 dns.home. alpn=dot port=853"`. Es el tipo con el que se publica una designación DDR en `_dns.resolver.arpa.` (RFC 9462) |
| 23 | `HTTPS` | La forma SVCB específica de HTTPS (RFC 9460 §9). Mismo formato de valor que `SVCB` |

## Respuestas negativas

Cuando el servidor es autoritativo para la zona de un nombre y no tiene nada con lo que responder, **cuál de los dos negativos devuelve lo decide el nombre, nunca el tipo por el que se preguntó**:

- **NXDOMAIN** afirma que el nombre consultado no existe. Solo se devuelve cuando no vive nada en el nombre ni por debajo de él.
- **NODATA** — NOERROR con la sección de respuesta vacía — afirma que el nombre existe y el tipo consultado no. Cubre cualquier otro fallo autoritativo: un nombre que tiene registros de otros tipos, y un no-terminal vacío que no tiene ninguno propio pero sí descendientes.

Ambos llevan el SOA del ápice de la zona que los engloba en la **sección de autoridad** (RFC 2308 §3), hallado subiendo desde el nombre consultado. Su MINIMUM, acotado por el TTL del propio registro, es como un resolutor aprende cuánto tiempo puede cachear la ausencia en vez de elegir un valor por defecto suyo. Ambos se marcan como autoritativos, que es lo que le dice a un resolutor que el negativo merece creerse en lugar de merecer preguntárselo a otro.

La distinción no es cosmética. Un host con registro A y sin AAAA debe responder NOERROR a uno y NODATA — no NXDOMAIN — al otro, porque bajo RFC 8020 un resolutor que cachea un NXDOMAIN para un nombre puede sintetizar NXDOMAIN para **todos los nombres por debajo de él**, así que una sola consulta AAAA para un host solo-v4 puede borrar su subárbol entero de la vista de ese resolutor.
...
La caché distingue dos clases de entradas:

- **Registros locales** (de la base de datos SQLite): cacheados en memoria con TTL estables (sin decaimiento). Estas entradas no se persisten al almacén de respaldo de la caché, ya que viven en la base de datos. La caché DNS en memoria se invalida automáticamente cada vez que se añaden, eliminan o modifican registros por gRPC, así que los cambios surten efecto de inmediato.
- **Respuestas reenviadas** (de los resolutores upstream): cacheadas con TTL decrecientes y persistidas en una tabla de caché respaldada por SQLite. Al reiniciar, las entradas persistidas se recargan, así que la caché está caliente de inmediato.

Las respuestas negativas (NXDOMAIN/NODATA autoritativos) se cachean aparte, durante el TTL negativo del RFC 2308 (`min(SOA MINIMUM, SOA TTL)`) tal como lo publicó la zona. Añadir un registro local para un nombre descarta cualquier negativo cacheado para él, así que un nombre recién añadido resuelve de inmediato en vez de esperar a que expire el TTL negativo.

Las estadísticas de la caché están disponibles por `GetCacheStats` y la caché se puede vaciar con `FlushDnsCache`.
...
| Modo | Comportamiento |
| ---- | -------- |
| `auto` (por defecto) | La cadena de reserva por niveles de más abajo |
| `recursive` | Iterativo solo desde los servidores raíz; no se contacta nunca con un resolutor upstream |
| `forward` | Reenvía solo a los `forwarders` configurados |

**El fichero de configuración es solo la semilla de arranque.** `resolution.mode` se
...
consultas. Los dos difieren tras un cambio — nunca se reinicia un servidor en marcha
para cambiar de modo, porque reiniciar el único resolutor de una máquina es una caída
...
**`arpa.` no se resuelve nunca fuera de esta máquina.** En todos los modos, `arpa.` y todo lo que hay bajo él se responde desde datos locales —un PTR almacenado, un registro con ámbito, una zona inversa gestionada o autoritativa— o se da **REFUSED**. Nada del subárbol se envía a un servidor raíz, a un reenviador ni a un upstream cifrado. REFUSED y no NXDOMAIN porque el servidor se está negando a responder por un espacio de nombres, no afirmando que el nombre no exista.
...
| 2 | Reenviador en texto plano que apunta a una dirección **privada** | El resolutor local/provisto por DHCP |
| 3 | Reenviador en texto plano que apunta a una dirección **pública** | Último recurso |
...
**En qué nivel cae un upstream se deriva de lo que es, no de bajo qué clave se escribió.** `forwarders`, `resolution.secure_upstreams` y `resolution.public_fallback` se analizan todos como el mismo tipo de reenviador, y cada uno se archiva por su transporte y su dirección: cifrado, texto-plano-privado o texto-plano-público. Una entrada DoH escrita en `forwarders:` se prueba primero porque está cifrada; una de `public_fallback` que apunta a la LAN se trata como reenviador local.

#### Especificaciones de reenviador

Un `ip:puerto` a secas sigue significando UDP en texto plano, así que todo fichero de configuración y todo cliente de la API escritos antes de esto se analizan sin cambios:

| Especificación | Transporte |
| -------------- | ---------- |
| `8.8.8.8:53` | UDP en texto plano (Do53) |
| `tcp://8.8.8.8:53` | TCP en texto plano (RFC 7766) |
...
La autoridad `name@ip` lleva en una sola cadena las dos mitades que necesita un upstream cifrado: la dirección a marcar y el nombre contra el que validar su certificado. Eso es lo que conserva la propiedad de arranque de que no haya que resolver ningún nombre de host antes de poder alcanzar un resolutor. Omitir `name@` valida contra los IP SAN de la propia dirección, que es una comprobación real y no una dispensa.
...
Cada reenviador lleva un cortacircuitos: tres fallos **de transporte** consecutivos lo abren, sigue abierto 30 segundos, luego se deja pasar una consulta para ver si se ha recuperado, y un éxito lo cierra del todo.

- **La salud se registra desde el transporte, no desde el rcode.** Un SERVFAIL que llegó es un resolutor que funciona respondiendo mal a una pregunta, y expulsarlo por una zona rota sería peor que no tener cortacircuitos.
- **La salud sobrevive a la reprogramación.** Volver a empujar una lista de reenviadores idéntica — cosa que un controlador puede hacer cada pocos segundos — traslada la salud existente a la lista nueva, en vez de entregar al servidor un cortacircuitos limpio más rápido de lo que tres fallos pueden dispararlo.
...
Un nivel solo «gana» cuando el transporte tuvo éxito y el rcode es NoError o NXDOMAIN; SERVFAIL, REFUSED y las respuestas no interpretables caen al siguiente. El nivel ganador es **pegajoso**, así que las consultas no pagan un tiempo de espera en un camino muerto cada vez. Recuperar un nivel más preferido ocurre de inmediato; degradar a uno menor solo se confirma tras `resolution.switch_grace_failures` consultas divergentes consecutivas, así que una consulta inestable no puede hacer oscilar el resolutor. **Las consultas de cliente nunca sondean**: el nivel de partida es siempre el nivel confirmado. Una tarea de fondo vuelve a probar los niveles por encima cada `resolution.recovery_probe_secs` con su propio canario desechable, y reclamar el nivel 0 exige una respuesta validada con DNSSEC para el propio `DNSKEY` de la zona raíz — la mera alcanzabilidad dejaría que cualquier caja intermedia que secuestre el `:53` se instalase como el nivel más fiable. Todo cambio de nivel confirmado vacía la caché DNS, así que las respuestas de un nivel no pueden quedarse tras un cambio a otro.

### Resolutor iterativo

El resolutor recorre la cadena de delegaciones desde las raíces —raíz → TLD → autoritativo— con el bit de recursión deseada apagado, validando las respuestas por identificador de transacción y nombre de la pregunta frente a la suplantación fuera de camino, sobre UDP con reserva automática a TCP al truncarse.
...
Bajo la caché de respuestas hay dos cachés que honran los TTL y guardan lo que una recursión aprende en el camino de bajada:

- **Caché de delegaciones** — zona → direcciones de servidores de nombres, aprendida de cada remisión. Una búsqueda de `.com` caliente se salta el salto a la raíz por completo. Las delegaciones cuyo TTL supera `resolution.delegation_persist_min_ttl` (300 s por defecto) se persisten en SQLite y se recargan al arrancar, así que un reinicio vuelve caliente; los conjuntos NS de la raíz y de los TLD traen TTL de varios días, así que sobreviven exactamente las entradas que merece la pena guardar.
...
**DNS-over-TLS (DoT)** — RFC 7858, puerto 853 por defecto, testigo ALPN `dot`. DNS estándar envuelto en TLS sobre TCP, con el mismo encuadre de prefijo de longitud de 2 bytes. El testigo ALPN se anuncia, no se exige: un cliente que ofrece `dot` lo negocia, un cliente que solo ofrece algún otro protocolo se rechaza, y a un cliente que no manda extensión ALPN alguna se le sirve igualmente. Se configura con la sección `dot` en YAML o con `SetDotConfig` por gRPC.

**DNS-over-HTTPS (DoH)** — RFC 8484, puerto 443 por defecto. Consultas DNS sobre HTTPS con soporte de los métodos GET (`/dns-query?dns=<base64>`) y POST (`application/dns-message`). Opcionalmente sirve HTTP/3 sobre QUIC en el mismo puerto (`enable_h3: true`), anunciado a los clientes ya conectados con una cabecera `Alt-Svc` y a todos los demás mediante el `alpn=h2,h3` de la designación DDR. Se configura con la sección `doh` en YAML o con `SetDohConfig` por gRPC.

**DNS-over-QUIC (DoQ)** — RFC 9250, puerto 8853 por defecto. Consultas DNS sobre transporte QUIC para resolución cifrada de baja latencia. Se configura con la sección `doq` en YAML o con `SetDoqConfig` por gRPC.

Los tres protocolos requieren certificados TLS. Puedes aportar tu propio certificado y clave, o poner `auto_self_signed: true` para que Rolodex DNS genere un certificado autofirmado automáticamente. Un certificado generado cubre `localhost`, `127.0.0.1`, `::1` y las propias direcciones de enlace de la escucha; añade cualquier otro nombre por el que los clientes marquen la máquina —su nombre de máquina, su nombre `.local`, un alias de la LAN— a `self_signed_sans`, ya que un cliente configurado con un nombre de autenticación lo comprueba y un enlace comodín no aporta ningún nombre propio.

**Se puede nombrar un certificado que aún no existe.** Que `cert_path`/`key_path` apunten a un fichero que no está ahí solo es un fallo duro cuando `auto_self_signed` está desactivado. Con él activado, la escucha arranca con material generado y el sondeo de certificados adopta el par real en cuanto aparece — sin reinicio y sin nada que coordinar. Eso es lo que permite configurar una escucha para un certificado que otra cosa todavía no ha emitido, que es el caso corriente en una máquina cuya CA se crea después de que arranque el resolutor. Con `auto_self_signed: false` un fichero ausente sigue siendo fatal: eso es quien opera diciendo «sirve este certificado o ninguno».

**Los tres se reconfiguran con el servidor en marcha.** `SetDotConfig`, `SetDohConfig` y `SetDoqConfig` abren, mueven, recambian la clave o apagan su escucha sin reiniciar, y `Get*Config` informa de las direcciones realmente enlazadas — que difieren de las pedidas siempre que la petición nombrara el puerto 0. El camino de arranque usa el mismo código, así que una configuración que funciona desde el YAML se comporta igual llegando por gRPC.

El orden merece conocerse, porque una escucha no puede arrancar antes de que la anterior suelte su puerto. Primero se comprueba todo lo comprobable **sin** el puerto — que la lista de enlaces resuelva, que el material TLS cargue o se genere — de modo que una dirección mala o un certificado ilegible se rechazan con la escucha anterior aún sirviendo. Solo después se paran las escuchas viejas y se esperan. Si aun así el enlace falla, se restaura la configuración anterior y la llamada informa de que el transporte está caído en vez de afirmar que tuvo éxito. Una lista de enlaces vacía es un apagado, no un error.
...
- **Ed25519** (preferido) — claves y firmas compactas, firma rápida
- **ECDSA P-256/SHA-256** y **ECDSA P-384/SHA-384**

**RSA/SHA-256 (algoritmo 8) no se puede generar** y `generate-dnssec-key` lo rechaza: `ring` no genera claves RSA. Sigue *interpretándose* —una fila de clave existente archivada bajo él se puede seguir listando— y las firmas RSA de zonas upstream son verificables, pero nada de aquí firmará con él. Un algoritmo que no se puede honrar de extremo a extremo se rechaza en la generación de claves en vez de sustituirse en silencio, porque un DNSKEY que anuncia un algoritmo sobre el material de clave de otro produce un DS, un DNSKEY y un conjunto de RRSIG que discrepan todos entre sí, y ese fallo aflora en un resolutor validador y no localmente.

Ed448 no está soportado por una limitación del crate de criptografía ring.

#### Flujo de trabajo de la firma

1. Genera una clave de firma de claves (KSK) y una clave de firma de zona (ZSK) para tu zona:
   ```bash
...
**Firmar es una instantánea, y una zona no sigue firmada por sí sola.** Un registro añadido después de `sign-zone` no tiene RRSIG — que un validador lee como una firma *arrancada* y no como un nombre sin firmar — y la cadena NSEC sigue describiendo la zona tal como era, así que la zona serviría una prueba firmada de que su propio registro nuevo no existe. Nada de eso es visible para quien añadió el registro.

No hace falta volver a ejecutar `sign-zone` por esto. Cada mutación de registro marca su zona, y un **bucle de refirmado vacía las marcas cada 30 segundos**. Va fuera de la ruta de mutación porque firmar es O(zona) y los registros llegan a ráfagas, de modo que una ráfaga cuesta una pasada de firma en lugar de una por registro. Una pasada fallida deja la marca puesta y la siguiente reintenta.
...
Volver a ejecutar `sign-zone` a mano sigue siendo la forma de forzar una pasada inmediata, y es lo que quieres tras un cambio de claves.

### Validación del upstream

Las respuestas resueltas **iterativamente** se validan contra las anclas de confianza de la raíz de IANA. Esto está activo por defecto:

```yaml
dnssec:
  validate: true        # el valor por defecto
...
La distinción que carga con la seguridad es Insecure frente a Bogus. «No hay firma» *no* es Insecure — un atacante en el camino quita las firmas de cualquier respuesta. Es Insecure solo cuando un NSEC/NSEC3 firmado demuestra el DS ausente en la delegación de arriba, algo que un atacante no puede falsificar sin la clave del padre. Esa demostración es la razón de existir de la maquinaria NSEC/NSEC3; sin ella, un validador es uno que un atacante degrada hasta dejarlo en nada.

Cómo se comporta en la práctica:

- **La cadena se construye de arriba abajo**, junto al recorrido de delegaciones que el resolutor ya hace, así que el DS viaja gratis en la remisión. Los conjuntos de claves validados (y las delegaciones demostradamente inseguras) se cachean por zona, así que una zona caliente no cuesta ninguna rederivación.
...
**Anclas de confianza.** `dnssec.trust_anchors` toma la forma de presentación DNSKEY — `"<flags> <protocolo> <algoritmo> <clave en base64>"`, los cuatro campos RDATA tal como los imprime `dig DNSKEY .`. Una sustitución **reemplaza** las claves de IANA en vez de añadirse a ellas, así que una raíz privada queda anclada a su propia clave y a nada más. Cada campo se valida al arrancar y un ancla malformada es un fallo duro, no una reserva silenciosa — un ancla que no puede casar con un DNSKEY real hace que toda zona firmada falle sin que nada apunte al ancla como causa.
...
- **Registros `CERT` (RFC 4398)** en `_ca.<zona>.` — un registro por certificado, con RDATA `"1 0 0 <DER en base64>"` (tipo 1 = PKIX/X.509, etiqueta de clave y algoritmo 0). La raíz se identifica como el certificado autofirmado. Vale cualquier cliente DNS:
  ```bash
...
En la red de confianza, el portal de inscripción (`acme.portal_bind`, por defecto `https://<host>:8500`) sirve la CA raíz en `GET /api/ca`, y la CLI de gestión imprime la cadena completa:
...
El prefijo por defecto es `64:ff9b::/96` (el prefijo NAT64 bien conocido). Por ejemplo, un registro A de `192.0.2.1` se sintetizaría como `64:ff9b::192.0.2.1` (`64:ff9b::c000:201`).
...
Una sección `metrics` opcional arranca un endpoint de raspado por HTTP en claro en `/metrics`. La sección está **ausente por defecto**, así que no se arranca escucha alguna y una actualización no abre ningún puerto nuevo.
...
El endpoint no está autenticado y solo lleva recuentos agregados — sin nombres de consulta, sin valores de registro, sin material de certificados. Enlázalo a una dirección privada; por defecto es loopback. TLS no se ofrece aquí deliberadamente, ya que significaría enviar un certificado autofirmado a cada raspador para un endpoint que, de entrada, no debería ser alcanzable públicamente.

Se exponen 85 familias de métricas, todas con el prefijo `rolodex_dns_`, que cubren las consultas, la caché de respuestas, las listas de bloqueo (incluidos los rechazos y los proveedores sacados de rotación), los niveles upstream, el resolutor iterativo, los veredictos DNSSEC, el estado del horizonte partido, DHCP, ACME, gRPC y el propio trabajo bloqueante del runtime.

La que conviene conocer es `rolodex_dns_answers_total{source}`, que informa de qué etapa del orden de resolución produjo cada respuesta — `cache`, `local`, `scoped`, `scope_fallback`, `tld_peer`, `blocklist`, `reverse_blocklist`, `dns64`, `upstream`, `authoritative_nxdomain`, `authoritative_nodata`, `refused`, `error`. Su total es igual al total de consultas, que es lo que hace legible la tubería de horizonte partido desde fuera:
...
3. **La lista almacenada.** Gestionada en tiempo de ejecución, sin reinicio:
...
# Ratio de aciertos de la caché de respuestas
...
Listas de bloqueo — el par que importa es bloqueos frente a rechazos, porque una lista que ha dejado de responder se ve idéntica a una limpia si solo se vigila el contador de bloqueos:
...
Bloqueo del runtime — dónde hay trabajo síncrono ocupando hilos que deberían estar sirviendo consultas. `db_lock_wait` y `db_locked` son las dos mitades de la única conexión SQLite: el tiempo de espera es lo que te cuestan otros llamantes, el tiempo de tenencia es lo que tú les cuestas a ellos.
...
# Coste del scrape como fracción de su intervalo: /metrics compitiendo con la
...
# Tiempo medio de verificar las firmas de un RRset, sobre todo el juego de claves candidatas
...
Ambas están desactivadas/vacías por defecto: no se consulta nada externo y no se entrega ningún nombre a un operador de listas de bloqueo hasta que se añaden proveedores.

### Base de datos de bloqueo local

Las entradas locales son la lista propia del operador, comprobada antes de consultar a proveedor alguno, y gestionada con `AddLocalBlocklistEntry`, `RemoveLocalBlocklistEntry` y `ListLocalBlocklistEntries`.
...
| Código | Significado |
| ---- | ------- |
| `127.255.255.0/24` | Rango de error de Spamhaus: `.252` errata en el nombre de zona, `.254` consulta a través de un resolutor público/abierto, `.255` consultas excesivas. Un rango entero en vez de los tres códigos, porque Spamhaus lo reserva y va añadiendo |
...
Poner un enfriamiento a `0` significa «usa el valor por defecto», no «sin enfriamiento» — un enfriamiento cero vuelve a preguntar al proveedor que acaba de decirte que pares, que es el comportamiento que la rotación existe para evitar.
...
DNSBL da a las listas de bloqueo **prioridad sobre el DNS externo**. La comprobación se ejecuta después de los registros locales y de las zonas gestionadas/autoritativas —así que los datos internos siempre ganan— pero **antes** de la caché de respuestas upstream y de cualquier resolución externa. Un nombre listado devuelve por tanto NXDOMAIN aunque antes se hubiera cacheado una respuesta reenviada para él.

DNSBL está desactivado por defecto con una lista de proveedores vacía, y los proveedores individuales se pueden activar o desactivar de forma independiente. Un DNSBL activado pero vacío no hace nada. Las zonas estándar que un operador suele añadir son `dbl.spamhaus.org`, `multi.surbl.org` y `multi.uribl.com`. Los resultados se cachean como arriba (los positivos durante el TTL del proveedor, los negativos 5 minutos).
...
La lista de permitidos es la salida de emergencia del operador ante un falso positivo, y cubre **todas las listas y ambas compuertas**: la comprobación del nombre directo (proveedores DNSBL y lista de bloqueo local) *y* la comprobación de DNS inverso/IP (entradas locales que nombran una dirección). Una IP listada por error rompe `dig -x` para una máquina que funciona bien, así que una salida de emergencia que solo llegara a los nombres no lo sería.
...
- **Cortocircuita la comprobación por completo.** Un nombre o dirección exento no se comprueba contra ningún proveedor y no emite búsqueda alguna a las listas de bloqueo.
- Las entradas se normalizan (minúsculas, punto final), así que cualquier grafía añade o elimina la misma entrada; persisten entre reinicios y surten efecto en la siguiente consulta sin necesidad de vaciar la caché.
...
- **Ámbito de red**: una vista DNS con nombre, con su propio conjunto de registros DNS y un dominio `.home` reservado (p. ej. `office.home.`). El dominio `.home` se usa como dominio de búsqueda por defecto para los clientes DHCP.
...
2. Añade registros DNS con ámbito al ámbito
...
### Orígenes de confianza frente a pares de superposición

La imposición de ámbito se aplica **solo** a las IP de origen que están dentro de `security.overlay_cidrs` (por defecto `10.64.0.0/10`, el rango de la superposición WireGuard). Un par así tiene que estar unido a una red o se le rechaza, y ve solo los TLD particionados de su propio ámbito. Cualquier otro origen es de confianza y resuelve la vista global.
...
Más allá de su dominio `.home` implícito, un ámbito puede poseer TLD adicionales que particionan el espacio de nombres entre redes. Cada TLD propio es **globalmente único** para un ámbito, y los nombres bajo él no se reenvían nunca upstream — un nombre sin coincidencia da un NXDOMAIN autoritativo, tras consultar opcionalmente los *reenviadores pares* del TLD (las direcciones de superposición de otros miembros Rolodex de la misma red).
...
Un ámbito puede por tanto existir puramente para poseer un TLD —marcándolo como particionado frente a los pares y resoluble desde la LAN— sin llegar a enlazar nunca una superposición a él.
...
1. **Enlaza una escucha DNS** (UDP + TCP) en esa IP, en `dns.ingress_listen_port` (53 por defecto). Las escuchas se vuelven a crear al arrancar a partir de la base de datos, y se desmontan cuando se elimina el último TLD que referencia la IP. Un enlace que falla —el caso habitual al arrancar, cuando la interfaz de superposición todavía no existe— se reintenta en el siguiente registro en vez de recordarse como «ya está escuchando».
2. **Sirve la vista del ámbito propietario para todos los nombres.** La escucha es el resolutor dedicado de esa red, así que una consulta que llega por ella pertenece al ámbito propietario sea cual sea el nombre: los TLD propios siguen particionados, y todo lo demás cae a la resolución global y a la resolución upstream — que es lo que permite a un par usarla como su resolutor de uso general.
...
2. Comprobar la lista de bloqueo local (para las consultas de DNS inverso)
3. Comprobar la caché de respuestas DNS
4. Comprobar los registros con ámbito del ámbito del cliente
5. Comprobar los registros CNAME con ámbito
6. Comprobar los registros DNAME con ámbito (reescritura de subárbol)
7. Comprobar si el nombre está bajo una zona gestionada con ámbito (negativo autoritativo: NXDOMAIN o NODATA, véase [Respuestas negativas](#respuestas-negativas))
8. Comprobar los registros globales de la base de datos
9. Comprobar los registros CNAME globales
10. Comprobar los registros DNAME globales (reescritura de subárbol)
11. Comprobar los registros ANAME (resolver el alias en el ápice de la zona)
12. Comprobar si el nombre está bajo una zona gestionada global (negativo autoritativo: NXDOMAIN o NODATA, véase [Respuestas negativas](#respuestas-negativas))
13. Comprobar los registros comodín (`*.zone.`)
14. Comprobar la lista de bloqueo local y los proveedores DNSBL (un nombre listado es NXDOMAIN, con prioridad sobre cualquier respuesta externa)
...
Rolodex DNS incluye un servidor DHCPv4 integrado con gestión de direcciones IP y registro DNS automático. Está desactivado salvo que haya una sección `dhcp` en la configuración.
...
- **Registro DNS automático.** Un cliente que manda un nombre de máquina (opción 12) obtiene un registro A en `<hostname>.lan.<dhcp.tld>.` y un PTR `in-addr.arpa` correspondiente, ambos como registros con ámbito en el ámbito del conjunto. La concesión además se une al ámbito de red (`JoinNetwork`), así que el cliente ve de inmediato la vista de horizonte partido de esa red. Ambos registros se eliminan cuando la concesión se libera o expira.
...
- **Entrega de certificados.** Los certificados se pueden entregar a los clientes mediante opciones DHCP específicas del sitio (códigos 224–254), configuradas por ámbito.
...
| Opción | Descripción |
|--------|-------------|
| `WithAuthToken(token)` | Fija el secreto compartido que se manda con cada RPC para la autenticación por TCP. El servidor lo ignora en las conexiones por socket Unix. Por defecto: vacío (funciona si el servidor no tiene secreto configurado) |
...
| `WithGRPCDialOption(opt)` | Añade una `grpc.DialOption` de bajo nivel (p. ej. para TLS o interceptores) |
...
| Método | Descripción |
|--------|-------------|
| `AddRecord(ctx, record) error` | Añade un registro DNS |
...
| `FlushCache(ctx) error` | Vacía la caché de las listas de bloqueo y devuelve a la rotación a todos los proveedores sacados de ella |
| `AddLocalBlocklistEntry(ctx, entry) error` | Añade una entrada a la lista de bloqueo local |
...
| `AddDnsblAllowlistEntry(ctx, entry) error` | Exime a un nombre (y a sus subdominios) de la comprobación de listas de bloqueo |
...
| `AddScopedRecord(ctx, scope, record) error` | Añade un registro DNS con ámbito |
...
| `AddScopeTldWithListener(ctx, scope, tld, listenIP) error` | Registra un TLD propio y enlaza una escucha DNS de ingreso |
...
| Método | Descripción |
|--------|-------------|
| `AddDhcpPool(ctx, pool) (string, error)` | Añade un conjunto de direcciones DHCP para un ámbito |
...
| Método | Descripción |
|--------|-------------|
| `GetCacheStats(ctx) (*CacheStats, error)` | Obtiene las estadísticas de la caché (entradas, aciertos, fallos) |
| `FlushDnsCache(ctx) error` | Vacía la caché de respuestas DNS |
...
| Método | Descripción |
|--------|-------------|
| `GenerateDnssecKey(ctx, zone, algorithm, keyType) (*DnssecKey, error)` | Genera un par de claves DNSSEC |
| `ListDnssecKeys(ctx, zone) ([]*DnssecKey, error)` | Lista las claves DNSSEC de una zona |
| `DeleteDnssecKey(ctx, keyID) error` | Elimina una clave DNSSEC |
| `GetDsRecords(ctx, zone) ([]string, error)` | Obtiene los registros DS para el registrador |
| `SignZone(ctx, zone) error` | Firma una zona con sus claves |
...
|----------|-------|-------------|
| `RecordTypeA` | 0 | Dirección IPv4 (por defecto) |
...
| `RecordTypeDNSKEY` | 15 | Clave pública DNSSEC |
...
2. Comprobar la lista de bloqueo local (para las consultas de DNS inverso)
3. Comprobar la caché de respuestas DNS
4. Comprobar la base de datos local (horizonte partido, siempre preferida)
5. Buscar registros CNAME en la base de datos local
6. Buscar registros DNAME (reescritura de subárbol)
7. Comprobar los registros ANAME (resolución del alias en el ápice de la zona)
8. Si el nombre está bajo una zona gestionada pero no se encuentra, devolver un negativo autoritativo: NXDOMAIN cuando no vive nada en el nombre ni por debajo, NODATA en caso contrario
9. Comprobar los registros comodín
10. Comprobar la lista de bloqueo local y los proveedores DNSBL (NXDOMAIN si está listado, por delante de cualquier respuesta externa)
...
Este proyecto está licenciado bajo la GNU Affero General Public License v3.0 (AGPL-3.0). Véase el fichero [LICENSE](LICENSE) para el texto completo de la licencia.

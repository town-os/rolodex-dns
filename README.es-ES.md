...
- **Firma DNSSEC**: generación de claves Ed25519 (preferido) y ECDSA P-256/P-384, firma de zonas y cálculo de registros DS. RSA/SHA-256 es verificable pero no se puede generar (`ring` no genera claves RSA), y la denegación autenticada (NSEC/NSEC3) no se produce
...
| `forwarders` | `["8.8.8.8:53", "8.8.4.4:53"]` | Direcciones de los resolutores DNS upstream (el nivel `local` en modo `auto`; el único upstream en modo `forward`) |
...
| 2 | Do53 en claro a `forwarders` | El resolutor local o provisto por DHCP |
...
La firma vuelve a publicar el RRset DNSKEY del ápice y produce un RRSIG por RRset. Vuelve a ejecutar `sign-zone` tras añadir o modificar registros; los RRSIG existentes se reemplazan en vez de acumularse.
...
7. Comprobar si el nombre está bajo una zona gestionada con ámbito (NXDOMAIN autoritativo)
...
12. Comprobar si el nombre está bajo una zona gestionada global (NXDOMAIN autoritativo)
...
8. Si el nombre está bajo una zona gestionada pero no se encuentra, devolver NXDOMAIN autoritativo

...
- **DNSSEC 署名**：Ed25519（推奨）および ECDSA P-256/P-384 の鍵生成、ゾーン署名、DS レコードの計算、そして認証付き否定のための NSEC チェーン。RSA/SHA-256 は検証はできますが生成はできません（`ring` に RSA 鍵の生成が無いため）。署名と証明は DO を立てたクライアントに提供され、再署名ループが、変更されたゾーンを手動の `sign-zone` に委ねることなく署名済みのまま保ちます
...
- **24 のレコード型**：A、AAAA、CNAME、MX、TXT、NS、SOA、SRV、PTR、URI、SSHFP、DNAME、ANAME、ZONEMD、TLSA、CERT、DNSKEY、DS、RRSIG、NSEC、NSEC3、NSEC3PARAM、SVCB、HTTPS。24 種すべてを保存・一覧できます。NSEC はゾーン署名が生成し、否定の証明として提供されます。NSEC3 と NSEC3PARAM は保存はできますが決して生成されません（[DNSSEC](#dnssec) を参照）
...
- **Prometheus メトリクス**：任意の、既定では無効な `/metrics` エンドポイント。ラベルの濃度が有界な 85 のメトリクスファミリを公開します —— 段階ごとの応答帰属や TLD ごとの分離を含むので、スプリットホライズンの流れが外から読み取れます。クエリ名がラベルになることはありません
...
# 上流 DNS フォワーダー。裸の address:port は従来どおり平文 UDP です。
# スキームを付けるとトランスポートが選ばれ、name@ip という authority が、
# ダイヤルする宛先アドレスと証明書を検証する名前の両方を与えます。
# エントリが auto のどの階層に入るかは、このキーではなくその
# トランスポートで決まります：ここに書いた暗号化フォワーダーはセキュア層で試されます。
...
| `forwarders` | `["8.8.8.8:53", "8.8.4.4:53"]` | 上流 DNS フォワーダー。各エントリは裸の `address:port`（平文 UDP）か、スキーム付きの仕様——`tcp://`、`tls://`、`https://`、`quic://`（[フォワーダー仕様](#フォワーダー仕様)を参照）。`forward` モードではこれらが唯一の上流です。`auto` モードでは各エントリが自分のトランスポートの示す階層に入るので、暗号化されたものは `local` 層ではなく `secure` 層で試されます |
...
実行中に上流 DNS フォワーダーを設定します。一覧全体を置き換えます。
...
| オプション | 既定 | 説明 |
|--------|---------|-------------|
| `-f, --forwarders <ADDR>...` | -- | 上流フォワーダーの仕様：裸の `"host:port"` は平文 UDP、スキーム付きの仕様は暗号化または TCP の上流です（[フォワーダー仕様](#フォワーダー仕様)を参照）。複数の仕様は空白で区切ります |
...
# 暗号化された上流。先にホスト名を解決しなくても到達できます。
# これらは auto チェーンのローカル層ではなくセキュア層に入ります
...
稼働中のサーバーで暗号化された上流を再設定する方法はこれだけです：
`resolution.secure_upstreams` は起動時に一度読まれるだけでセッターが無く、
外向きの `:53` を遮断するネットワークでは、唯一動く階層が、その筐体で唯一の
リゾルバーを再起動しない限り何も変えられない階層でもあったからです。
...
**パラメータ：**
- `forwarders`（repeated string）：上流フォワーダーの仕様。裸の `"host:port"` は平文 UDP です（例 `"8.8.8.8:53"`）。スキームを付けるとトランスポートが選ばれます——`"tcp://8.8.8.8:53"`、`"tls://cloudflare-dns.com@1.1.1.1:853"`、`"https://cloudflare-dns.com@1.1.1.1/dns-query"`、`"quic://dns.adguard.com@94.140.14.14:853"`（[フォワーダー仕様](#フォワーダー仕様)を参照）。一覧はどれ一つ適用される前に全体が解析され、不正な仕様があれば呼び出しは `INVALID_ARGUMENT` で失敗します——4 番目のエントリの打ち間違いのせいで半分だけ入った一覧は、呼び出し側が求めてもおらず見ることもできない状態にリゾルバーを置きます
- `auth_token`（string）：認証のための共有秘密
...
| 22 | `SVCB` | サービスバインディング（RFC 9460）。値は表現形式の一行：`"<優先度> <ターゲット> [キー=値 ...]"` —— 例 `"1 dns.home. alpn=dot port=853"`。DDR の指定を `_dns.resolver.arpa.` に公開するときの型です（RFC 9462）|
| 23 | `HTTPS` | HTTPS 専用の SVCB 形式（RFC 9460 §9）。値の形式は `SVCB` と同じ |

## 否定応答

サーバーがある名前のゾーンに対して権威を持ち、返せるものが何も無いとき、**どちらの否定を返すかを決めるのは名前であって、尋ねられた型では決してありません**：

- **NXDOMAIN** は、問い合わせられた名前が存在しないと主張します。その名前にも、その配下にも何も無いときにだけ返されます。
- **NODATA**——NOERROR で応答セクションが空——は、名前は存在し、問い合わせられた型が存在しないと主張します。それ以外のあらゆる権威的な空振りがこちらです：別の型のレコードを持つ名前や、自分自身のレコードは無いが子孫を持つ空の非終端ノードです。

どちらも、それを含むゾーンの頂点 SOA を**権威セクション**に載せます（RFC 2308 §3）。SOA は問い合わせられた名前から上へ辿って見つけます。その MINIMUM（当のレコード自身の TTL が上限）が、リゾルバーがこの「不在」をどれだけキャッシュしてよいかを、自前の既定値を選ぶことなく学ぶ手立てです。どちらも権威ありとして印が付きます。それこそが、この否定は誰か他人に聞き直すに値するものではなく信じるに値するのだと、リゾルバーに伝えるものです。

この区別は飾りではありません。A レコードを持ち AAAA を持たないホストは、一方に NOERROR を、もう一方には NXDOMAIN ではなく NODATA を返さなければなりません。RFC 8020 のもとでは、ある名前の NXDOMAIN をキャッシュしたリゾルバーは**その配下のすべての名前**について NXDOMAIN を合成してよいので、v4 のみのホストへの AAAA クエリ一回で、そのリゾルバーの視界からサブツリーごと消え得るからです。

知っておく価値のある二つの場合：

- **ワイルドカードが一致する名前は存在します。** RFC 4592 §2.2.1 はワイルドカードにその名前を*確立*させるので、ワイルドカードが持たない型を尋ねれば NODATA です。そのまま否定すれば、同じワイルドカードが一つ前のクエリで返した肯定応答と矛盾します。ワイルドカードが一致するのは 1 ラベルなので、その二段下の名前は何にも確立されず NXDOMAIN のままです。
- **別のネットワークスコープが所有する TLD は、そこに何があろうと否定されます。** サーバーが自分の知っている名前に対して NXDOMAIN を返す唯一の場所であり、これは分割ホライズンの分割が設計どおり働いている姿です：NODATA は「名前は実在し、足りないのは型だけだ」と主張してしまい、それこそがあるスコープに、NXDOMAIN ではなく NODATA が返る名前を観察することで隣接ネットワークの名前を列挙させてしまいます。

署名済みゾーンでは、DO を立てたクライアントはこの否定の NSEC 証明も受け取ります——[クライアントが受け取るもの](#クライアントが受け取るもの)を参照。

`rolodex_dns_authoritative_negative_total{reason}` は、この探索が**何を見たか**を報告します：`name_absent`、`type_absent`、`unsupported_type`、`scope_hidden`。
...
| 1 | **暗号化された**フォワーダーすべて：DoH（`:443`）、DoT（`:853`）、DoQ（`:853/UDP`） | 暗号化されており、`:53` の遮断を生き延びるポートを使います |
| 2 | **プライベート**アドレスを指す平文フォワーダー | ローカルの、あるいは DHCP が配ったリゾルバー |
| 3 | **パブリック**アドレスを指す平文フォワーダー | 最後の手段 |

DoT より DoH が好まれるのは、`:443` がふつうの HTTPS に見え、DoT の接続は開かせておいてその TLS セッションを落とすような深層パケット検査を生き延びるからです。暗号化された上流は **IP で**接続され、証明書は仕様に書かれた名前に対して検証されるので、この階層は起動に先行する DNS を必要としません。

**上流がどの階層に入るかは、それが何であるかから決まり、どのキーの下に書かれたかでは決まりません。** `forwarders`、`resolution.secure_upstreams`、`resolution.public_fallback` はいずれも同じ種類のフォワーダーとして解析され、それぞれのトランスポートとアドレスによって分類されます：暗号化、平文プライベート、平文パブリックです。`forwarders:` に書かれた DoH エントリは暗号化されているので最初に試され、LAN を指す `public_fallback` のエントリはローカルのフォワーダーとして扱われます。

#### フォワーダー仕様

裸の `ip:port` は今も平文 UDP を意味するので、これ以前に書かれた設定ファイルも API 呼び出し側も、そのまま解析できます：
...
`name@ip` という authority は、暗号化された上流が必要とする二つの半分——ダイヤルする宛先アドレスと、その証明書を検証する名前——を一つの文字列で運びます。これが、リゾルバーに到達する前にホスト名を解決する必要がないという起動時の性質を保っています。`name@` を省くとアドレス自身の IP SAN に対して検証され、これは免除ではなく本物の検査です。

同じ文法は `SetForwarders` と `rolodex-dns-cli set-forwarders` でも受け付けられます。これこそが、暗号化された上流を実行中に再設定できるようにしているものです。

#### 失敗し続けるフォワーダーは試されなくなる

各フォワーダーはサーキットブレーカーを持ちます：**トランスポート**の失敗が三回続くと開き、30 秒開いたままになり、その後、回復したかを見るために一つだけクエリが通され、成功すれば完全に閉じます。

- **健全性は rcode ではなくトランスポートから記録されます。** 届いた SERVFAIL は、正常に動いているリゾルバーが一つの問いに下手に答えただけであり、壊れたゾーン一つのために追い出すのはブレーカーが無いより悪いことです。
- **健全性は再設定を生き延びます。** まったく同じフォワーダー一覧を押し直しても——コントローラーは数秒ごとにそれをしかねません——既存の健全性が新しい一覧へ引き継がれます。そうしないと、三回の失敗が引き金を引くより速く、まっさらなブレーカーがサーバーに渡り続けます。
- **`forward` モードは決してスキップしません。** そこではフォワーダーが上流のすべてなので、不調が続いたからと飛ばすのは、より良いものへ退くことではなく解決を拒むことになります。健全性の記録は続けられ、モードが戻ったときに使われます。

これが、死んだ階層を安くしているものです。`auto` は SERVFAIL を返せるようになるまでチェーン全体を歩くので、あるトランスポートをブラックホール化するネットワークでは、答えるはずのない二つの階層のアドレスすべてについて、毎回 1.5 秒のタイムアウトを丸ごと払っていました。省かれたやり取りは `rolodex_dns_upstream_skipped_total{server}` に数えられます。
...
**まだ存在しない証明書を名指せます。** `cert_path`／`key_path` がそこに無いファイルを指していることが強い失敗になるのは、`auto_self_signed` が切れているときだけです。入れておけば、リスナーは生成された材料で起動し、証明書の巡回が、本物の組が現れたその時にそれを引き継ぎます —— 再起動は要らず、調整すべきことも何もありません。これによって、他の何かがまだ発行していない証明書に向けてリスナーを設定できます。CA がリゾルバーの起動のあとに作られる機体では、それがごく普通の場合です。`auto_self_signed: false` のもとでは、不在のファイルは今も致命的です。それは運用者が「この証明書を出すか、さもなくば何も出すな」と言っているのです。

**三つとも、サーバーを動かしたまま設定し直せます。** `SetDotConfig`、`SetDohConfig`、`SetDoqConfig` は、再起動なしにそれぞれのリスナーを開き、移し、鍵を替え、あるいは停止します。そして `Get*Config` が報告するのは実際にバインドされたアドレスであり、要求がポート 0 を名指していれば、それは要求されたものとは異なります。起動の経路も同じコードを使うので、YAML から効く設定は、gRPC で届いても同じように振る舞います。

その順序は知っておく価値があります。古いリスナーがポートを手放すまで、新しいものは起動できないからです。まずポート**なしで**確かめられることをすべて確かめます —— バインド一覧が解決すること、TLS の材料が読めるか生成できること —— そうすることで、悪いアドレスや読めない証明書は、古いリスナーがまだ提供している状態で拒否されます。そのあとで初めて古いリスナーを止め、その終了を待ちます。それでもバインドが失敗したら、前の設定を戻し、呼び出しは成功したと称するのではなくそのトランスポートが落ちていると報告します。空のバインド一覧はエラーではなく停止です。

この一連のどこでも `:53` は触れられません。暗号化トランスポートは互いに独立したリスナーであり、片方を設定し直す代価は、その外側には及びません。
...
署名は頂点の DNSKEY RRset を公開し直し、RRset ごとに一つの RRSIG を作り、同じパスでゾーンの **NSEC チェーン**も構築して署名します。既存の RRSIG は積み上がるのではなく置き換えられます。

DNSKEY、DS、RRSIG はそれぞれの型コードで提供され、その RDATA は署名側がハッシュするのと同じ正準符号化器が作ります —— 線上へ出るものは、署名されたものとバイト単位で同一です。

NSEC3 と NSEC3PARAM は今も保存・一覧のできるレコード型ですが、それらを生成するものはありません：ここでの認証付き否定は NSEC です。

#### クライアントが受け取るもの

署名と証明が提供される条件は二つあり、どちらも省けません：クライアントが **DO ビット**を立てていること、そしてその名前が、このサーバーが鍵を持つゾーンの内側にあることです。誰も求めていない署名は A 応答の大きさをおよそ三倍にし、小さな問いへの大きな答えは、`security.recursion_cidrs` が塞ぐために存在している増幅の形そのものです。

- **肯定応答**は、その RRset を覆う RRSIG を応答セクションの RRset の隣に載せます（RFC 4035 §3.1.1）。
- **NODATA** は、問い合わせられた名前自身の NSEC を載せます。その型ビットマップは何が存在するかを列挙しており、問い合わせられた型がその一覧に無いことが否定そのものです。
- **NXDOMAIN** は二つの証明を載せます：その名前を覆うチェーンのリンクと、`*.<最近接包含者>` を覆うリンクです。後者が無いと、ワイルドカードが応答を合成していた可能性をバリデーターは排除できず、RFC 4035 §5.4 によりその否定は拒否されます。
- **否定応答の SOA も署名されます。** バリデーターは否定 TTL を読むのと同じセクションでそれを認証するからであり、混在した権威セクションは全体として失敗します。
- **ワイルドカード由来の肯定応答には意図的に署名が付きません。** レコードは `*.parent` にあり、問い合わせられた名前へ書き換えられるため、それを覆う RRSIG はワイルドカードの所有者名に置かれています。RRset を署名なしで出せばバリデーターは *insecure* に留まりますが、所有者の合わない署名を付けて出せば *bogus* になり、そちらが厳密に悪いのです。
- **スコープ付きレコードは決して署名も証明もされません。** 署名はグローバルテーブルに対して働くので、スコープ付きの否定は SOA だけを載せ、他には何も載せません。

#### 署名済みゾーンを署名済みのまま保つ

**署名はスナップショットであり、ゾーンは自分だけで署名済みのままではいられません。** `sign-zone` の後に追加されたレコードには RRSIG が無く——バリデーターはそれを未署名の名前ではなく*剥ぎ取られた*署名として読みます——さらに NSEC チェーンは変更前のゾーンを記述したままなので、ゾーンは自分自身の新しいレコードが存在しないという署名付きの証明を出すことになります。そのどれもが、レコードを追加した人には見えません。

そのために `sign-zone` を再実行する必要はありません。レコードの変更はそのゾーンに印を付け、**再署名ループが 30 秒ごとに印を消化します**。署名は O(ゾーン) であり、レコードはまとまって到着するため、これは変更経路の外で走ります——ひとまとまりにつき署名一巡で済み、レコードごとに払う必要がありません。失敗したパスは印を残し、次のパスが再試行します。

ゾーンに印が付いている間、**否定は古い証明を付けてではなく、証明なしで出ていきます**：証明の無い否定はバリデーターを *insecure* に留めますが、変更より古いチェーンは *bogus* にしてゾーン全体を落とします。`rolodex_dns_denial_proofs_withheld_total` を見てください——署名済みゾーンが今まさに証明なしで答えていることを示す指標です。

手で `sign-zone` を実行し直すのは今も直ちに一巡を強制する方法であり、鍵を変えた後にやりたいのはこれです。
...
85 のメトリクスファミリが公開されます。いずれも `rolodex_dns_` の接頭辞を持ち、クエリ、応答キャッシュ、ブロックリスト（拒否とローテーションから外れたプロバイダーを含む）、上流の階層、反復リゾルバー、DNSSEC の判定、スプリットホライズンの状態、DHCP、ACME、gRPC、そしてランタイム自身のブロッキング処理を覆います。

知っておく価値があるのは `rolodex_dns_answers_total{source}` です。これは、解決順序のどの段階が各応答を生んだかを報告します —— `cache`、`local`、`scoped`、`scope_fallback`、`tld_peer`、`blocklist`、`reverse_blocklist`、`dns64`、`upstream`、`authoritative_nxdomain`、`authoritative_nodata`、`refused`、`error`。その合計はクエリの合計に等しく、それがスプリットホライズンの流れを外から読めるものにしています：
...
7. 名前がスコープ付きの管理下ゾーンの配下かを確認（権威ある否定——NXDOMAIN または NODATA。[否定応答](#否定応答)を参照）
8. 全体のデータベースのレコードを確認
9. 全体の CNAME レコードを確認
10. 全体の DNAME レコードを確認（サブツリーの書き換え）
11. ANAME レコードを確認（ゾーン頂点で別名を解決）
12. 名前が全体の管理下ゾーンの配下かを確認（権威ある否定——NXDOMAIN または NODATA。[否定応答](#否定応答)を参照）
...
| `RecordTypeSVCB` | 22 | サービスバインディング（RFC 9460）。DDR の指定が使う型 |
| `RecordTypeHTTPS` | 23 | HTTPS 専用の SVCB 形式（RFC 9460 §9）|
...
8. 名前が管理下ゾーンの配下にあって見つからなければ、権威ある否定を返す——その名前にも配下にも何も無ければ NXDOMAIN、そうでなければ NODATA
9. ワイルドカードレコードを確認

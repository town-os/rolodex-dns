...
- **DNSSEC signing**: Ed25519 (preferred) and ECDSA P-256/P-384 key generation, zone signing, DS record computation, and an NSEC chain for authenticated denial. RSA/SHA-256 is verifiable but cannot be generated (`ring` has no RSA key generation). Signatures and proofs are served to clients that set DO, and a re-sign loop keeps a mutated zone signed rather than leaving it to a manual `sign-zone`
...
- **24 record types**: A, AAAA, CNAME, MX, TXT, NS, SOA, SRV, PTR, URI, SSHFP, DNAME, ANAME, ZONEMD, TLSA, CERT, DNSKEY, DS, RRSIG, NSEC, NSEC3, NSEC3PARAM, SVCB, HTTPS. All 24 can be stored and listed; NSEC is generated by zone signing and served as denial proof, while NSEC3 and NSEC3PARAM are storable but never generated (see [DNSSEC](#dnssec))
...
# Upstream DNS forwarders. A bare address:port is plaintext UDP, exactly as
# before; a scheme selects the transport, and the name@ip authority gives both
# the address to dial and the name to validate the certificate against.
# Which auto tier an entry lands in follows from its transport, not from this
# key: an encrypted forwarder here is tried in the secure tier.
...
| `forwarders` | `["8.8.8.8:53", "8.8.4.4:53"]` | Upstream DNS forwarders, each a bare `address:port` (plaintext UDP) or a scheme-typed spec — `tcp://`, `tls://`, `https://`, `quic://` (see [Forwarder Specs](#forwarder-specs)). In `forward` mode these are the only upstream; in `auto` mode each entry lands in the tier its transport implies, so an encrypted one is tried in the `secure` tier rather than the `local` one |
...
| Option | Default | Description |
|--------|---------|-------------|
| `-f, --forwarders <ADDR>...` | -- | Upstream forwarder specs: a bare `"host:port"` for plaintext UDP, or a scheme-typed spec for an encrypted or TCP upstream (see [Forwarder Specs](#forwarder-specs)). Multiple specs separated by spaces |
...
# Encrypted upstreams, reachable without resolving a hostname first.
# These land in the secure tier of the auto chain, not the local one
...
This is the only way to reconfigure an encrypted upstream on a running server:
`resolution.secure_upstreams` is read once at startup and has no setter, so on a
network that filters outbound `:53` the one tier that worked used to be the one
tier nothing could change without restarting the box's only resolver.
...
Configures upstream DNS forwarders at runtime. Replaces the whole list.

**Parameters:**
- `forwarders` (repeated string): Upstream forwarder specs. A bare `"host:port"` is plaintext UDP (e.g. `"8.8.8.8:53"`); a scheme selects the transport — `"tcp://8.8.8.8:53"`, `"tls://cloudflare-dns.com@1.1.1.1:853"`, `"https://cloudflare-dns.com@1.1.1.1/dns-query"`, `"quic://dns.adguard.com@94.140.14.14:853"` (see [Forwarder Specs](#forwarder-specs)). The whole list is parsed before any of it is applied, and a malformed spec fails the call with `INVALID_ARGUMENT` — a list half-installed because its fourth entry was a typo leaves the resolver in a state the caller did not ask for and cannot see
- `auth_token` (string): Shared secret for authentication
...
When the server is authoritative for a name's zone and has nothing to answer with, **which negative it returns is decided by the name, never by the type that was asked for**:

- **NXDOMAIN** asserts the queried name does not exist. It is returned only when nothing lives at the name or beneath it.
- **NODATA** — NOERROR with an empty answer section — asserts the name exists and the queried type does not. It covers every other authoritative miss: a name holding records of other types, and an empty non-terminal that holds none of its own but has descendants.

Both carry the enclosing zone's apex SOA in the **authority section** (RFC 2308 §3), found by walking up from the queried name. Its MINIMUM, capped by the record's own TTL, is how a resolver learns how long it may cache the absence instead of picking a default of its own. Both are flagged authoritative, which is what tells a resolver the negative is worth believing rather than worth asking someone else about.

The distinction is not cosmetic. A host with an A record and no AAAA must answer NOERROR for one and NODATA — not NXDOMAIN — for the other, because under RFC 8020 a resolver that caches NXDOMAIN for a name may synthesize NXDOMAIN for **every name beneath it**, so one AAAA query for a v4-only host can erase its whole subtree from that resolver's view.

Two cases are worth knowing about:

- **A name a wildcard matches exists.** RFC 4592 §2.2.1 has the wildcard establish the name, so asking it for a type the wildcard does not carry is NODATA. Denying it outright would contradict the positive answer the same wildcard gives one query earlier. A wildcard matches one label, so a name two levels below it is established by nothing and is NXDOMAIN.
- **A TLD owned by a different network scope is denied whatever exists there.** This is the one place the server returns NXDOMAIN for a name it knows about, and it is the split-horizon partition working as designed: NODATA would assert the name is real and only the type missing, which is exactly what would let one scope enumerate a sibling network's names by watching which came back NODATA instead of NXDOMAIN.

For a signed zone, a client that set DO also receives the NSEC proof of the denial — see [What a Client Receives](#what-a-client-receives).

`rolodex_dns_authoritative_negative_total{reason}` reports what the lookup found: `name_absent`, `type_absent`, `unsupported_type` or `scope_hidden`.
...
| 1 | Any **encrypted** forwarder: DoH (`:443`), DoT (`:853`) or DoQ (`:853/UDP`) | Encrypted, and uses ports that survive `:53` filtering |
| 2 | Plaintext forwarder pointing at a **private** address | The local/DHCP-provided resolver |
| 3 | Plaintext forwarder pointing at a **public** address | Last resort |

DoH is preferred over DoT because `:443` looks like ordinary HTTPS and survives deep-packet inspection that lets a DoT connection open but drops its TLS session. Encrypted upstreams are dialed **by IP**, with the certificate validated against the name given in the spec, so the tier needs no prior DNS to bootstrap.

**Which tier an upstream lands in follows from what it is, not from which key it was written under.** `forwarders`, `resolution.secure_upstreams` and `resolution.public_fallback` all parse into the same kind of forwarder, and each is filed by its transport and address: encrypted, plaintext-private, or plaintext-public. A DoH entry typed into `forwarders:` is tried first because it is encrypted; a `public_fallback` entry pointing at the LAN is treated as a local forwarder.

#### Forwarder Specs

A bare `ip:port` still means plaintext UDP, so every config file and API caller written before this parses unchanged:
...
The `name@ip` authority carries both halves an encrypted upstream needs — the address to dial, and the name to validate its certificate against — in one string. That is what keeps the bootstrap property that no hostname has to be resolved before a resolver can be reached. Omitting `name@` validates against the address's own IP SANs, which is a real check rather than a waiver.

The same grammar is accepted by `SetForwarders` and `rolodex-dns-cli set-forwarders`, which is what makes encrypted upstreams reconfigurable at runtime at all.

#### A Forwarder That Keeps Failing Stops Being Tried

Each forwarder carries a circuit breaker: three consecutive **transport** failures open it, it stays open for 30 seconds, then one query is let through to see whether it recovered, and a success closes it outright.

- **Health is recorded from the transport, not the rcode.** A SERVFAIL that arrived is a working resolver answering one question badly, and evicting it over one broken zone would be worse than having no breaker.
- **Health survives reprogramming.** Re-pushing an identical forwarder list — which a controller may do every few seconds — carries the existing health onto the new list, rather than handing the server a clean breaker faster than three failures can trip it.
...
Signing republishes the apex DNSKEY RRset, produces one RRSIG per RRset, and builds and signs the zone's **NSEC chain** in the same pass. Existing RRSIGs are replaced rather than accumulated.
...
NSEC3 and NSEC3PARAM remain storable and listable record types, but nothing generates them: authenticated denial here is NSEC.

#### What a Client Receives

Signatures and proofs are served on two conditions, and neither is optional: the client set the **DO bit**, and the name falls inside a zone this server holds keys for. A signature nobody asked for roughly triples the size of an A answer, and a large answer to a small question is the amplification shape `security.recursion_cidrs` exists to close.

- **Positive answers** carry the RRSIGs covering the RRset, beside it in the answer section (RFC 4035 §3.1.1).
- **NODATA** carries the queried name's own NSEC, whose type bitmap lists what is present — the queried type's absence from that list is the denial.
- **NXDOMAIN** carries two proofs: the chain link covering the name, and the link covering `*.<closest encloser>`. Without the second, a validator cannot rule out that a wildcard would have synthesized an answer, and RFC 4035 §5.4 has it reject the denial.
- **The SOA in a negative is signed too**, since a validator authenticates the negative TTL from the same section it reads it in, and a mixed authority section fails as a whole.
- **Wildcard-derived positive answers carry no signature**, deliberately. The records live at `*.parent` and are rewritten to the queried name, so the covering RRSIG sits at the wildcard's owner. Serving the RRset unsigned leaves a validator at *insecure*; serving it under a signature whose owner does not match would leave it at *bogus*, which is strictly worse.
- **Scoped records are never signed or proven.** Signing operates on the global table, so a scoped negative carries its SOA and nothing else.

#### Keeping a Signed Zone Signed

**Signing is a snapshot, and a zone does not stay signed on its own.** A record added after `sign-zone` has no RRSIG — which a validator reads as a *stripped* signature rather than an unsigned name — and the NSEC chain still describes the zone as it was, so the zone would serve a signed proof that its own new record does not exist. Nothing about that is visible to whoever added the record.

You do not have to re-run `sign-zone` for this. Every record mutation marks its zone, and a **re-sign loop drains the marks every 30 seconds**. It runs off the mutation path because signing is O(zone) and records arrive in bursts, so a burst costs one signing pass rather than one per record. A failed pass leaves the mark in place and the next one retries.

While a zone is marked, **negatives go out without their proof rather than with a stale one**: an unproven negative leaves a validator at *insecure*, where a chain that predates the mutation would leave it at *bogus* and take the whole zone down. Watch `rolodex_dns_denial_proofs_withheld_total` — it is the metric that says a signed zone is currently answering unproven.

Re-running `sign-zone` by hand is still the way to force a pass immediately, and is what you want after a key change.
...
7. Check if name is under a scoped managed zone (authoritative negative — NXDOMAIN or NODATA, see [Negative Answers](#negative-answers))
...
12. Check if name is under a global managed zone (authoritative negative — NXDOMAIN or NODATA, see [Negative Answers](#negative-answers))
...
8. If name is under a managed zone but not found, return an authoritative negative — NXDOMAIN when nothing lives at or beneath the name, NODATA otherwise

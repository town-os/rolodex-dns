...
- **DNSSEC 签名**：Ed25519（首选）与 ECDSA P-256/P-384 的密钥生成、区域签名、DS 记录计算，以及用于认证否定证明的 NSEC 链。RSA/SHA-256 可验证但无法生成（`ring` 没有 RSA 密钥生成功能）。签名与证明会提供给设置了 DO 的客户端，而重新签名循环会让被变更过的区域保持已签名的状态，不必依赖手动的 `sign-zone`
...
- **24 种记录类型**：A、AAAA、CNAME、MX、TXT、NS、SOA、SRV、PTR、URI、SSHFP、DNAME、ANAME、ZONEMD、TLSA、CERT、DNSKEY、DS、RRSIG、NSEC、NSEC3、NSEC3PARAM、SVCB、HTTPS。全部 24 种都可存储与列出；NSEC 由区域签名生成并作为否定证明提供，而 NSEC3 与 NSEC3PARAM 可存储但永远不会被生成（见 [DNSSEC](#dnssec)）
...
# 上游 DNS 转发器。裸写的 address:port 一如既往就是明文 UDP；
# 加上 scheme 就会选定传输方式，而 name@ip 这个 authority 同时给出
# 要拨号的地址与要用来验证证书的名称。
# 一条条目会落在 auto 的哪一层，是由它的传输方式决定的，不是由这个键：
# 写在这里的加密转发器会在安全层被尝试。
...
| `forwarders` | `["8.8.8.8:53", "8.8.4.4:53"]` | 上游 DNS 转发器，每一条可以是裸写的 `address:port`（明文 UDP），也可以是带 scheme 的规格——`tcp://`、`tls://`、`https://`、`quic://`（见[转发器规格](#转发器规格)）。在 `forward` 模式下这些就是唯一的上游；在 `auto` 模式下，每一条会落在其传输方式所决定的层级，因此加密的那一条会在 `secure` 层而不是 `local` 层被尝试 |
...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| `-f, --forwarders <ADDR>...` | -- | 上游转发器规格：裸写的 `"host:port"` 表示明文 UDP，带 scheme 的规格则表示加密或 TCP 的上游（见[转发器规格](#转发器规格)）。多条规格以空格分隔 |
...
要在一台正在跑的服务器上重新配置加密上游，这是唯一的办法：
`resolution.secure_upstreams` 只在启动时读取一次，而且没有设置方法，
所以在过滤掉对外 `:53` 的网络上，唯一能用的那一层，过去同时也是
不重启这台机器唯一的解析器就什么都改不了的那一层。
...
在运行期设置上游 DNS 转发器。会取代整份清单。

**参数：**
- `forwarders`（repeated string）：上游转发器规格。裸写的 `"host:port"` 是明文 UDP（例如 `"8.8.8.8:53"`）；加上 scheme 就会选定传输方式——`"tcp://8.8.8.8:53"`、`"tls://cloudflare-dns.com@1.1.1.1:853"`、`"https://cloudflare-dns.com@1.1.1.1/dns-query"`、`"quic://dns.adguard.com@94.140.14.14:853"`（见[转发器规格](#转发器规格)）。整份清单会在套用任何一条之前先解析完毕，只要有一条格式错误，整个调用就会以 `INVALID_ARGUMENT` 失败——一份因为第四条打错字而只装了一半的清单，会让解析器停在一个调用方既没要求、也看不见的状态
- `auth_token`（string）：认证用的共享密钥
...
当服务器对某个名称所属的区域具权威、却没有东西可以回答时，**它送出哪一种否定答案，是由名称决定的，绝不是由被问的类型决定的**：

- **NXDOMAIN** 主张被查询的名称不存在。只有在该名称以及它底下什么都没有时才会返回。
- **NODATA**——NOERROR 且答案区段为空——主张这个名称存在，而被问的类型不存在。其余每一种权威性的落空都算在这里：一个持有其他类型记录的名称，以及一个自己没有任何记录、却有后代的空非终端节点。

两者都会在**授权区段**带上所属区域顶点的 SOA（RFC 2308 §3），做法是从被查询的名称往上走。它的 MINIMUM（并以该记录自身的 TTL 为上限）就是解析器用来得知这个“不存在”可以缓存多久的依据，而不必自己挑一个默认值。两者也都会被标记为权威，而这正是告诉解析器“这个否定答案值得相信，而不是值得再去问别人”的东西。

这个区别不是修辞。一台有 A 记录、没有 AAAA 的主机，必须对其中一个回 NOERROR，对另一个回 NODATA——不是 NXDOMAIN——因为依 RFC 8020，一个把某名称的 NXDOMAIN 缓存起来的解析器，可以据此为**它底下的每一个名称**合成 NXDOMAIN，于是对一台只有 v4 的主机做一次 AAAA 查询，就能在那个解析器眼中抹掉它的整个子树。

有两种情况值得知道：

- **通配符匹配到的名称是存在的。** RFC 4592 §2.2.1 让通配符**确立**了该名称，因此拿一个它并不持有的类型去问它，得到的是 NODATA。直接否定它，会和同一个通配符前一个查询才给出的肯定答案自相矛盾。通配符只匹配一个标签，所以位于它底下两层的名称不被任何东西确立，仍然是 NXDOMAIN。
- **由另一个网络范围拥有的 TLD，无论那里有什么都会被否定。** 这是服务器唯一一处对自己知道的名称回 NXDOMAIN 的地方，而这正是分割视域的分区设计本意：NODATA 会主张这个名称是真的、只是缺了那个类型，而那恰恰会让一个范围借由观察哪些名称回的是 NODATA 而不是 NXDOMAIN，把邻近网络的名称一个个列举出来。

对于已签名的区域，设置了 DO 的客户端还会收到这个否定的 NSEC 证明——见[客户端会收到什么](#客户端会收到什么)。

`rolodex_dns_authoritative_negative_total{reason}` 会报告这次查找**看到了什么**：`name_absent`、`type_absent`、`unsupported_type` 或 `scope_hidden`。
...
| 1 | 任何**加密**的转发器：DoH（`:443`）、DoT（`:853`）或 DoQ（`:853/UDP`） | 已加密，且使用的端口能撑过 `:53` 过滤 |
| 2 | 指向**私有**地址的明文转发器 | 本地／由 DHCP 提供的解析器 |
| 3 | 指向**公开**地址的明文转发器 | 最后手段 |

DoH 优先于 DoT，因为 `:443` 看起来就像普通的 HTTPS，能撑过那种“让 DoT 连接建立起来、却把它的 TLS 会话丢掉”的深度包检测。加密上游是**以 IP** 拨号，并用规格里给定的名称验证证书，因此这个层级启动时不需要任何先行的 DNS。

**一个上游落在哪一层，是由它是什么决定的，不是由它被写在哪个键底下决定的。** `forwarders`、`resolution.secure_upstreams` 与 `resolution.public_fallback` 都会解析成同一种转发器，然后各自依其传输方式与地址被归档：加密的、明文私有的，或明文公开的。打进 `forwarders:` 的一条 DoH 条目会最先被尝试，因为它是加密的；而一条指向局域网的 `public_fallback` 条目会被当成本地转发器。

#### 转发器规格

裸写的 `ip:port` 依然表示明文 UDP，因此在此之前写好的每一份配置文件与每一个 API 调用方都能照旧解析：
...
`name@ip` 这个 authority 用一个字符串同时带着加密上游需要的两半——要拨号的地址，以及要用来验证其证书的名称。这正是“在能连上一台解析器之前，不必先解析任何主机名”这个启动性质得以保留的原因。省略 `name@` 会改为对该地址自己的 IP SAN 验证，那是一次真正的检查，而不是放行。

同一套文法也被 `SetForwarders` 与 `rolodex-dns-cli set-forwarders` 接受，而这正是加密上游终于能在运行期重新配置的原因。

#### 一再失败的转发器会停止被尝试

每个转发器都带着一个断路器：连续三次**传输层**失败会把它打开，维持 30 秒，然后放一个查询过去看看它恢复了没有，成功就把它彻底关上。

- **健康状态是从传输层记录的，不是从 rcode。** 已经回到手上的 SERVFAIL 是一台正常运作的解析器把某一个问题答坏了，为了一个坏掉的区域就把它逐出，会比完全没有断路器更糟。
- **健康状态会撑过重新配置。** 重新推送一份一模一样的转发器清单——控制器可能每隔几秒就做一次——会把已有的健康状态带到新清单上，而不是让服务器拿到一个全新的断路器，快到三次失败根本来不及把它跳开。
- **`forward` 模式从不跳过。** 在那里转发器就是全部的上游，所以因为一段连败就略过其中一个，等于是拒绝解析，而不是退回到更好的东西。健康状态仍然会被记录，以备模式再切回来时使用。

这正是让一个死掉的层级变便宜的关键。`auto` 要走完整条链才能回答 SERVFAIL，所以在一个把某种传输方式黑洞掉的网络上，过去每个查询都得为两个根本不会回答的层级里的每一个地址，付出完整的 1.5 秒超时。被省下来的交换次数计入 `rolodex_dns_upstream_skipped_total{server}`。
...
签名会重新发布顶点的 DNSKEY RRset、为每个 RRset 生成一条 RRSIG，并在同一轮中建立并签署该区域的 **NSEC 链**。已有的 RRSIG 是被取代而不是累积。

DNSKEY、DS 与 RRSIG 以它们自己的类型码提供，RDATA 由签名者拿去做哈希的同一个规范编码器生成——送上线路的东西与被签的东西逐字节相同。

NSEC3 与 NSEC3PARAM 仍然是可存储、可列出的记录类型，但没有任何东西会生成它们：这里的认证否定证明就是 NSEC。

#### 客户端会收到什么

签名与证明的提供有两个条件，而且缺一不可：客户端设置了 **DO 位**，而且该名称落在本服务器持有密钥的区域里。没人要求的签名会把一条 A 答案的大小大约变成三倍，而“小问题换来大答案”正是 `security.recursion_cidrs` 存在要堵住的放大形状。

- **肯定答案**会在答案区段中，于 RRset 旁边带上覆盖它的 RRSIG（RFC 4035 §3.1.1）。
- **NODATA** 会带上被查询名称自己的 NSEC，它的类型位图列出了“存在的是什么”——被问的类型不在那份清单里，就是这个否定。
- **NXDOMAIN** 会带上两份证明：覆盖该名称的那一段链接，以及覆盖 `*.<最近包围者>` 的那一段。少了第二份，验证器就无法排除“有个通配符本来会合成出一个答案”，而 RFC 4035 §5.4 会让它因此拒绝这个否定。
- **否定答案里的 SOA 同样会被签名**，因为验证器是从它读取否定 TTL 的同一个区段里去认证它的，而一个混合的授权区段是整段一起失败的。
- **由通配符衍生出来的肯定答案刻意不带签名。** 那些记录存放在 `*.parent`，再被改写成被查询的名称，因此覆盖它们的 RRSIG 是落在通配符自己的所有者名称上。把 RRset 不带签名送出去，会让验证器停在 *insecure*；把它配上一个所有者名称对不上的签名送出去，则会让它落到 *bogus*，那严格来说更糟。
- **范围化的记录永远不会被签名，也不会被证明。** 签名作用于全局数据表，因此范围化的否定答案只会带着它的 SOA，别的什么都没有。

#### 让已签名的区域保持已签名

**签名是一张快照，而一个区域不会自己保持在已签名的状态。** 在 `sign-zone` 之后新增的一条记录没有 RRSIG——验证器会把那读成*被剥除*的签名，而不是一个未签名的名称——而且 NSEC 链描述的仍然是变动前的那个区域，于是这个区域会送出一份“已签名的、证明它自己的新记录不存在”的证明。这一切对新增记录的人来说完全看不见。

为此你不需要重跑 `sign-zone`。每一次记录变动都会给它的区域做上标记，而**重新签名循环每 30 秒清掉这些标记**。它跑在变动路径之外，因为签名的成本是 O(区域大小)，而记录是成批到达的——一整批只需付一次签名成本，而不是每条记录各付一次。失败的一轮会把标记留在原地，由下一轮重试。

在区域仍带着标记的期间，**否定答案会不带证明送出，而不是带着一份过期的**：一个未经证明的否定只让验证器停在 *insecure*，而一条早于这次变动的链会让它落到 *bogus*，把整个区域拖下线。请盯着 `rolodex_dns_denial_proofs_withheld_total`——它就是那个“这个已签名的区域目前正在给出未经证明的答案”的指标。

用手重跑 `sign-zone` 仍然是立刻强制跑一轮的办法，而那也正是换过密钥之后你会想做的事。
...
7. 检查名称是否位于某个范围内的受管区域之下（权威性的否定答案——NXDOMAIN 或 NODATA，见[否定答案](#否定答案)）
8. 检查全局数据库记录
9. 检查全局 CNAME 记录
10. 检查全局 DNAME 记录（子树改写）
11. 检查 ANAME 记录（在区域顶点解析别名）
12. 检查名称是否位于某个全局受管区域之下（权威性的否定答案——NXDOMAIN 或 NODATA，见[否定答案](#否定答案)）
13. 检查通配符记录（`*.zone.`）
...
8. 若名称位于某个受管区域之下却找不到，返回一个权威性的否定答案——该名称与其底下都没有东西时是 NXDOMAIN，否则是 NODATA
9. 检查通配符记录

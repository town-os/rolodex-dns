...
第一次接觸？請從 **[設定指南](CONFIGURATION.zh-Hant.md)** 開始——那是一份任務導向的逐步說明，從最小可用設定一路走到每個子系統，並為每種部署形態附上實作範例。
...
若想要一份逐個子系統把設定建立起來、並為每種部署形態附上實作範例的逐步說明，請見 **[設定指南](CONFIGURATION.zh-Hant.md)**。下面的參考是完整的欄位清單。

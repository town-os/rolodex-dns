...
- **DNSSEC 簽章**：Ed25519（首選）與 ECDSA P-256/P-384 的金鑰產生、區域簽章、DS 記錄計算，以及用於認證否定證明的 NSEC 鏈。RSA/SHA-256 可驗證但無法產生（`ring` 沒有 RSA 金鑰產生功能）。簽章與證明會提供給有設定 DO 的用戶端，而重新簽署迴圈會讓被變更過的區域維持已簽章的狀態，不必仰賴手動的 `sign-zone`
...
- **24 種記錄型別**：A、AAAA、CNAME、MX、TXT、NS、SOA、SRV、PTR、URI、SSHFP、DNAME、ANAME、ZONEMD、TLSA、CERT、DNSKEY、DS、RRSIG、NSEC、NSEC3、NSEC3PARAM、SVCB、HTTPS。全部 24 種都可儲存與列出；NSEC 由區域簽章產生並作為否定證明提供，而 NSEC3 與 NSEC3PARAM 可儲存但永遠不會被產生（見 [DNSSEC](#dnssec)）
...
- **Prometheus 指標**：一個選用、預設關閉的 `/metrics` 端點，輸出 85 個具備有界標籤基數的指標系列——包含逐階段的答案歸因與逐 TLD 隔離，讓分割視域管線從外面看得懂。查詢名稱永遠不會成為標籤
...
# 上游 DNS 轉送器。裸寫的 address:port 一如既往就是純文字 UDP；
# 加上 scheme 就會選定傳輸方式，而 name@ip 這個 authority 同時給出
# 要撥接的位址與要用來驗證憑證的名稱。
# 一筆項目會落在 auto 的哪一層，是由它的傳輸方式決定的，不是由這個鍵：
# 寫在這裡的加密轉送器會在安全層被嘗試。
...
| `forwarders` | `["8.8.8.8:53", "8.8.4.4:53"]` | 上游 DNS 轉送器，每一筆可以是裸寫的 `address:port`（純文字 UDP），也可以是帶 scheme 的規格——`tcp://`、`tls://`、`https://`、`quic://`（見[轉送器規格](#轉送器規格)）。在 `forward` 模式下這些就是唯一的上游；在 `auto` 模式下，每一筆會落在其傳輸方式所決定的層級，因此加密的那一筆會在 `secure` 層而不是 `local` 層被嘗試 |
...
| 選項 | 預設值 | 說明 |
|------|--------|------|
| `-f, --forwarders <ADDR>...` | -- | 上游轉送器規格：裸寫的 `"host:port"` 表示純文字 UDP，帶 scheme 的規格則表示加密或 TCP 的上游（見[轉送器規格](#轉送器規格)）。多筆規格以空白分隔 |
...
要在一台正在跑的伺服器上重新設定加密上游，這是唯一的辦法：
`resolution.secure_upstreams` 只在啟動時讀取一次，而且沒有設定方法，
所以在過濾掉對外 `:53` 的網路上，唯一能用的那一層，過去同時也是
不重啟這台機器唯一的解析器就什麼都改不了的那一層。
...
在執行期設定上游 DNS 轉送器。會取代整份清單。

**參數：**
- `forwarders`（repeated string）：上游轉送器規格。裸寫的 `"host:port"` 是純文字 UDP（例如 `"8.8.8.8:53"`）；加上 scheme 就會選定傳輸方式——`"tcp://8.8.8.8:53"`、`"tls://cloudflare-dns.com@1.1.1.1:853"`、`"https://cloudflare-dns.com@1.1.1.1/dns-query"`、`"quic://dns.adguard.com@94.140.14.14:853"`（見[轉送器規格](#轉送器規格)）。整份清單會在套用任何一筆之前先剖析完畢，只要有一筆格式錯誤，整個呼叫就會以 `INVALID_ARGUMENT` 失敗——一份因為第四筆打錯字而只裝了一半的清單，會讓解析器停在一個呼叫端既沒要求、也看不見的狀態
- `auth_token`（string）：認證用的共用密鑰
...
當伺服器對某個名稱所屬的區域具權威、卻沒有東西可以回答時，**它送出哪一種否定答案，是由名稱決定的，絕不是由被問的型別決定的**：

- **NXDOMAIN** 主張被查詢的名稱不存在。只有在該名稱以及它底下什麼都沒有時才會回傳。
- **NODATA**——NOERROR 且答案區段為空——主張這個名稱存在，而被問的型別不存在。其餘每一種權威性的落空都算在這裡：一個持有其他型別記錄的名稱，以及一個自己沒有任何記錄、卻有後代的空非終端節點。

兩者都會在**授權區段**帶上所屬區域頂點的 SOA（RFC 2308 §3），做法是從被查詢的名稱往上走。它的 MINIMUM（並以該記錄自身的 TTL 為上限）就是解析器用來得知這個「不存在」可以快取多久的依據，而不必自己挑一個預設值。兩者也都會被標記為權威，而這正是告訴解析器「這個否定答案值得相信，而不是值得再去問別人」的東西。

這個區別不是修辭。一台有 A 記錄、沒有 AAAA 的主機，必須對其中一個回 NOERROR，對另一個回 NODATA——不是 NXDOMAIN——因為依 RFC 8020，一個把某名稱的 NXDOMAIN 快取起來的解析器，可以據此為**它底下的每一個名稱**合成 NXDOMAIN，於是對一台只有 v4 的主機做一次 AAAA 查詢，就能在那個解析器眼中抹掉它的整個子樹。

有兩種情況值得知道：

- **通配符匹配到的名稱是存在的。** RFC 4592 §2.2.1 讓通配符**確立**了該名稱，因此拿一個它並不持有的型別去問它，得到的是 NODATA。直接否定它，會和同一個通配符前一個查詢才給出的肯定答案自相矛盾。通配符只匹配一個標籤，所以位於它底下兩層的名稱不被任何東西確立，仍然是 NXDOMAIN。
- **由另一個網路範圍擁有的 TLD，無論那裡有什麼都會被否定。** 這是伺服器唯一一處對自己知道的名稱回 NXDOMAIN 的地方，而這正是分割視域的分區設計本意：NODATA 會主張這個名稱是真的、只是缺了那個型別，而那恰恰會讓一個範圍藉由觀察哪些名稱回的是 NODATA 而不是 NXDOMAIN，把鄰近網路的名稱一個個列舉出來。

對於已簽章的區域，有設定 DO 的用戶端還會收到這個否定的 NSEC 證明——見[用戶端會收到什麼](#用戶端會收到什麼)。

`rolodex_dns_authoritative_negative_total{reason}` 會回報這次查找**看到了什麼**：`name_absent`、`type_absent`、`unsupported_type` 或 `scope_hidden`。
...
| 1 | 任何**加密**的轉送器：DoH（`:443`）、DoT（`:853`）或 DoQ（`:853/UDP`） | 已加密，且使用的連接埠能撐過 `:53` 過濾 |
| 2 | 指向**私有**位址的純文字轉送器 | 本地／由 DHCP 提供的解析器 |
| 3 | 指向**公開**位址的純文字轉送器 | 最後手段 |

DoH 優先於 DoT，因為 `:443` 看起來就像普通的 HTTPS，能撐過那種「讓 DoT 連線建立起來、卻把它的 TLS 工作階段丟掉」的深度封包檢測。加密上游是**以 IP** 撥接，並用規格裡給定的名稱驗證憑證，因此這個層級啟動時不需要任何先行的 DNS。

**一個上游落在哪一層，是由它是什麼決定的，不是由它被寫在哪個鍵底下決定的。** `forwarders`、`resolution.secure_upstreams` 與 `resolution.public_fallback` 都會剖析成同一種轉送器，然後各自依其傳輸方式與位址被歸檔：加密的、純文字私有的，或純文字公開的。打進 `forwarders:` 的一筆 DoH 項目會最先被嘗試，因為它是加密的；而一筆指向區域網路的 `public_fallback` 項目會被當成本地轉送器。

#### 轉送器規格

裸寫的 `ip:port` 依然表示純文字 UDP，因此在此之前寫好的每一份設定檔與每一個 API 呼叫端都能照舊剖析：

| 規格 | 傳輸方式 |
| ---- | -------- |
| `8.8.8.8:53` | 純文字 UDP（Do53） |
| `tcp://8.8.8.8:53` | 純文字 TCP（RFC 7766） |
...
`name@ip` 這個 authority 用一個字串同時帶著加密上游需要的兩半——要撥接的位址，以及要用來驗證其憑證的名稱。這正是「在能連上一台解析器之前，不必先解析任何主機名稱」這個啟動性質得以保留的原因。省略 `name@` 會改為對該位址自己的 IP SAN 驗證，那是一次真正的檢查，而不是放行。

同一套文法也被 `SetForwarders` 與 `rolodex-dns-cli set-forwarders` 接受，而這正是加密上游終於能在執行期重新設定的原因。

#### 一再失敗的轉送器會停止被嘗試

每個轉送器都帶著一個斷路器：連續三次**傳輸層**失敗會把它打開，維持 30 秒，然後放一個查詢過去看看它恢復了沒有，成功就把它徹底關上。

- **健康狀態是從傳輸層記錄的，不是從 rcode。** 已經回到手上的 SERVFAIL 是一台正常運作的解析器把某一個問題答壞了，為了一個壞掉的區域就把它逐出，會比完全沒有斷路器更糟。
- **健康狀態會撐過重新設定。** 重新推送一份一模一樣的轉送器清單——控制器可能每隔幾秒就做一次——會把既有的健康狀態帶到新清單上，而不是讓伺服器拿到一個全新的斷路器，快到三次失敗根本來不及把它跳開。
- **`forward` 模式從不跳過。** 在那裡轉送器就是全部的上游，所以因為一段連敗就略過其中一個，等於是拒絕解析，而不是退回到更好的東西。健康狀態仍然會被記錄，以備模式再切回來時使用。

這正是讓一個死掉的層級變便宜的關鍵。`auto` 要走完整條鏈才能回答 SERVFAIL，所以在一個把某種傳輸方式黑洞掉的網路上，過去每個查詢都得為兩個根本不會回答的層級裡的每一個位址，付出完整的 1.5 秒逾時。被省下來的交換次數計入 `rolodex_dns_upstream_skipped_total{server}`。
...
簽章會重新發佈頂點的 DNSKEY RRset、為每個 RRset 產生一筆 RRSIG，並在同一輪中建立並簽署該區域的 **NSEC 鏈**。既有的 RRSIG 是被取代而不是累積。

DNSKEY、DS 與 RRSIG 以它們自己的型別碼提供，RDATA 由簽章者拿去做雜湊的同一個正規編碼器產生——送上線路的東西與被簽的東西逐位元組相同。

NSEC3 與 NSEC3PARAM 仍然是可儲存、可列出的記錄型別，但沒有任何東西會產生它們：這裡的認證否定證明就是 NSEC。

#### 用戶端會收到什麼

簽章與證明的提供有兩個條件，而且缺一不可：用戶端設定了 **DO 位元**，而且該名稱落在本伺服器持有金鑰的區域裡。沒人要求的簽章會把一筆 A 答案的大小大約變成三倍，而「小問題換來大答案」正是 `security.recursion_cidrs` 存在要堵住的放大形狀。

- **肯定答案**會在答案區段中，於 RRset 旁邊帶上涵蓋它的 RRSIG（RFC 4035 §3.1.1）。
- **NODATA** 會帶上被查詢名稱自己的 NSEC，它的型別點陣圖列出了「存在的是什麼」——被問的型別不在那份清單裡，就是這個否定。
- **NXDOMAIN** 會帶上兩份證明：涵蓋該名稱的那一段鏈結，以及涵蓋 `*.<最近包圍者>` 的那一段。少了第二份，驗證器就無法排除「有個通配符本來會合成出一個答案」，而 RFC 4035 §5.4 會讓它因此拒絕這個否定。
- **否定答案裡的 SOA 同樣會被簽署**，因為驗證器是從它讀取否定 TTL 的同一個區段裡去認證它的，而一個混合的授權區段是整段一起失敗的。
- **由通配符衍生出來的肯定答案刻意不帶簽章。** 那些記錄存放在 `*.parent`，再被改寫成被查詢的名稱，因此涵蓋它們的 RRSIG 是落在通配符自己的擁有者名稱上。把 RRset 不帶簽章送出去，會讓驗證器停在 *insecure*；把它配上一個擁有者名稱對不上的簽章送出去，則會讓它落到 *bogus*，那嚴格來說更糟。
- **範圍化的記錄永遠不會被簽署，也不會被證明。** 簽章作用於全域資料表，因此範圍化的否定答案只會帶著它的 SOA，別的什麼都沒有。

#### 讓已簽章的區域維持已簽章

**簽章是一張快照，而一個區域不會自己維持在已簽章的狀態。** 在 `sign-zone` 之後新增的一筆記錄沒有 RRSIG——驗證器會把那讀成*被剝除*的簽章，而不是一個未簽章的名稱——而且 NSEC 鏈描述的仍然是變動前的那個區域，於是這個區域會送出一份「已簽署的、證明它自己的新記錄不存在」的證明。這一切對新增記錄的人來說完全看不見。

為此你不需要重跑 `sign-zone`。每一次記錄變動都會替它的區域做上記號，而**重新簽署迴圈每 30 秒清掉這些記號**。它跑在變動路徑之外，因為簽章的成本是 O(區域大小)，而記錄是成批到達的——一整批只需付一次簽章成本，而不是每筆記錄各付一次。失敗的一輪會把記號留在原地，由下一輪重試。

在區域仍帶著記號的期間，**否定答案會不帶證明送出，而不是帶著一份過期的**：一個未經證明的否定只讓驗證器停在 *insecure*，而一條早於這次變動的鏈會讓它落到 *bogus*，把整個區域拖下線。請盯著 `rolodex_dns_denial_proofs_withheld_total`——它就是那個「這個已簽章的區域目前正在給出未經證明的答案」的指標。

用手重跑 `sign-zone` 仍然是立刻強制跑一輪的辦法，而那也正是換過金鑰之後你會想做的事。
...
輸出 85 個指標系列，全部以 `rolodex_dns_` 為前綴，涵蓋查詢、回應快取、封鎖清單（包含拒答與被移出輪替的供應商）、上游層級、迭代解析器、DNSSEC 判定、分割視域狀態、DHCP、ACME、gRPC 與執行期本身的阻塞工作。

其中最值得認識的是 `rolodex_dns_answers_total{source}`，它回報解析順序中的哪個階段產生了每個答案——`cache`、`local`、`scoped`、`scope_fallback`、`tld_peer`、`blocklist`、`reverse_blocklist`、`dns64`、`upstream`、`authoritative_nxdomain`、`authoritative_nodata`、`refused`、`error`。它的總數等於查詢總數，而這正是讓分割視域管線從外面看得懂的關鍵：
...
7. 檢查名稱是否位於某個範圍內的受管區域之下（權威性的否定答案——NXDOMAIN 或 NODATA，見[否定答案](#否定答案)）
8. 檢查全域資料庫記錄
9. 檢查全域 CNAME 記錄
10. 檢查全域 DNAME 記錄（子樹改寫）
11. 檢查 ANAME 記錄（在區域頂點解析別名）
12. 檢查名稱是否位於某個全域受管區域之下（權威性的否定答案——NXDOMAIN 或 NODATA，見[否定答案](#否定答案)）
13. 檢查萬用字元記錄（`*.zone.`）
...
8. 若名稱位於某個受管區域之下卻找不到，回傳一個權威性的否定答案——該名稱與其底下都沒有東西時是 NXDOMAIN，否則是 NODATA
9. 檢查萬用字元記錄
